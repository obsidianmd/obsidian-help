---
permalink: credits
---
Mená nie sú zoradené podľa výšky príspevku, ale abecedne (alebo sa o to aspoň snažíme!).

## Hlavný tím

Hlavný tím, ktorý vytvára a udržiava Obsidian.

- codedmart (Brandon Martin)
- dharam (Dharam Kapila)
- kepano ([Steph Ango](https://stephanango.com/))
- Licat (Shida Li)
- liam ([Liam Cain](https://liamca.in/)) <span class='flair'>Maker of Calendar</span> <span class='flair'>Plugin API Master</span> <span class='flair'>Volcano Veteran</span>
- Silver ([Erica Xu](https://ericaxu.com))<span class='flair'>Team Little Silvers</span>
- Sigrunixia ([Rebbecca Bishop](https://tenebrousdragon.com/home))<span class='flair'>Sorcerous Support Dragon</span> <span class='flair'>TTRPG Enthusiast</span>
- tgrosinger ([Tony Grosinger](https://grosinger.net))

## Prispievatelia

Vývojári na čiastočný úväzok, ktorí pomáhajú vytvárať Obsidian.

- cmptr (Aaron Bell)
- mgmeyers ([Matthew Meyers](https://matthewmeye.rs/))

## Komunitná revízia

Prispievatelia do procesu a nástrojov revízie [komunitných](https://community.obsidian.md/) pluginov/tém.

- Fevol
- mnaoumov ([Michael Naumov](https://mnaoumov.dev/))
- saberzero1 ([Emile Bangma](https://emilebangma.com)) <span class='flair'>Quartz Quartermaster</span> <span class='flair'>ESLint Legislator</span>
- Zachatoo ([Zach Young](https://zachyoung.dev/)) <span class='flair'>Templater Templar</span><span class='flair'>Board Gamer</span>

## Moderovanie

Moderátori spravujú Obsidian Fórum, Discord a Reddit.

- argentum<span class='flair'>Team Little Silvers</span> <span class='flair'>Linux master</span> <span class='flair'>Zotero master</span> ^a4b3a2
- CawlinTeffid ([Colin Tedford](https://colintedford.com/)) <span class='flair'>embedded-search ambassador</span>
- cotemaxime ([Maxime Côté](https://www.maximecote.me/))<span class='flair'>Linux master</span>
- koala<span class='flair'>Extreme Bug Finder</span>
- Leah ([Leah Ferguson](http://leahferguson.com))<span class='flair'>Roll for initiative!</span> <span class='flair'>Non-techy Apple geek</span>
- rigmarole ([Chris Lesage](http://rigmarolestudio.com))<span class='flair'>Forum master</span>
- Roam ([Roman Soriano](https://leftovers.blog/)) <span class='flair'>Not <i>that</i> one</span>
- Trainbuffer <span class='flair'> ferroequinologist</span> <span class='flair'>Travels by rail</span>
- WhiteNoise<span class='flair'>Think Tank</span> <span class='flair'>Zotero master</span> <span class='flair'>Bug report exterminator</span>
- Carl the Turtle<span class='flair'>Online 24/7</span> <span class='flair'>Easily triggered</span> <span class='flair'>Good and pure</span> 

## Alumni

Bývalí moderátori a členovia tímu, ktorí sa vydali na nové dobrodružstvá!

- Eleanor ([Eleanor Konik](https://eleanorkonik.com))<span class='flair'>Halp Obsidian turned me into a dev</span>
- joethei ([Johannes Theiner](https://joethei.xyz/)) <span class='flair'>Plugin API Master</span>
- mediapathic ([Steen Comer](http://mediapathic.net/))<span class='flair'>Bad Cop</span>
- ryanjamurphy ([Ryan](https://fulcra.design/) [J. A. Murphy](https://axle.design/))<span class='flair'>That funny guy</span> <span class='flair'>Apple Fanboy</span> <span class='flair'>Forum master</span>

## Inšpirácie pluginov

Poďakovanie patrí týmto vývojárom pluginov za priekopnícku funkcionalitu, ktorá bola adaptovaná do Obsidian:

- pjeby za plugin Hotkey Helper
- javalent za plugin Admonition
- javalent za plugin Settings Search
- NothingIsLost za vylepšenia CodeMirror 6
- mgmeyers za plugin Indentation Guides
- blacksmithgu za Dataview

## Zostavy

- jstone za údržbu našich zostáv na Flathub
- adamgibbins a ran-dall za údržbu našich zostáv na homebrew

## Prekladatelia

Pozrite si tiež úplný zoznam [prispievateľov do prekladov](https://github.com/obsidianmd/obsidian-translations/graphs/contributors?from=7%2F31%2F2020).

- Andrea Brandi (taliančina)
- Anne-Katrin aka Mephi (nemčina)
- bongudth (vietnamčina)
- canzi-teacher, 蚕子 (zjednodušená čínština)
- cor-bee, Ілля Пишняк (ukrajinčina)
- cvyl, Mikka (holandčina)
- Daniel Mathiot (francúzština)
- Diego Tres (brazílska portugalčina)
- Haitham Mohamed (arabčina)
- Henrik Falk (dánčina)
- Joffrey Schroeder (francúzština) 
- JxhnnyUt8h (ruština)
- k-andzhanovskii, Константин Анджановский (ruština)
- lisachev, Сергей Лисачев (ruština)
- mafsi, Patrick Danilevici (rumunčina)
- Manuel Arias Almada (španielčina)
- Moyf, 墨一 (zjednodušená čínština)
- Muhammad Mauli Mubassari (indonézština)
- Olivier Anthonioz (francúzština) 
- Ryooo，睿悠ryooo (zjednodušená čínština)
- Sakvipubp Suy (khmérčina)
- smilorent, Silviu Lorenț (rumunčina)
- Solyta Teng (khmérčina)
- taksas, 中山 拓海 (japončina)
- VKondakoff, Валерий Кондаков (ruština)
- yo-goto, ぱだわん (japončina)
- Zachary Shirmohammadli (perzština) 

## Podporujúce organizácie

Organizácie, ktoré podporujú Obsidian s [[Komerčná licencia|komerčnou licenciou]], sú uvedené na našej stránke [Enterprise](https://obsidian.md/enterprise/). 

## Dokumentácia

Prvú verziu trezoru s pomocníkom napísala Silver. Neskôr časť napísal [@mediapathic](http://mediapathic.net) s veľkou pomocou od Reggieho. Tiež sme dostali opravy od mnohých pozorných očí v komunite na Discorde a fóre.

Dokumentáciu istý čas spravoval [Marcus Olsson](https://marcus.se.net/). V súčasnosti ju udržiava tím Obsidian.

Teraz prijímame pull requesty do trezoru s pomocníkom v [našom repozitári dokumentácie](https://github.com/obsidianmd/obsidian-docs/). Nahlásenia problémov sú vítané.

### Pridajte svoje meno

Ak ste prekladateľ, vytvorte prosím pull request a pridajte „Vaše meno (jazyk príspevku)" do sekcie vyššie. Prosím, zachovajte abecedné zoradenie zoznamu mien, ďakujeme!

Ak vaše meno nie je v angličtine a nedá sa abecedne zoradiť, použite tento formát: `váš-github-username, [vaše meno vo vašom jazyku] (Názov jazyka)`.

## Poďakovania tretím stranám

Obsidian používa nasledujúce open source knižnice (v abecednom poradí):

### Capacitor

Verzia `5.x`
MIT License
Copyright (c) 2017-present Drifty Co.

### CodeMirror

Vim Verzia `6.0.0`
MIT License
Copyright (C) 2018 by Marijn Haverbeke <marijnh@gmail.com>, Adrian Heine <mail@adrianheine.de>, and others

### DOMPurify

https://github.com/cure53/DOMPurify
Licencované pod [Mozilla Public License version 2.0](http://mozilla.org/MPL/2.0/)

### Electron

Verzia `37.3.0`
MIT License
Copyright (c) Electron contributors
Copyright (c) 2013-2020 GitHub Inc.

### i18next

MIT License
Copyright (c) 2022 i18next

### Lezer

MIT License
Copyright (C) 2018-2021 by Marijn Haverbeke <marijnh@gmail.com> and others

### Lucide

Verzia `0.446.0`
ISC License
Copyright (c) 2020, Lucide Contributors ^lucide

### MathJax

Apache License 2.0

### Mermaid

Verzia `11.4.1`
MIT License
Copyright (c) 2014 - 2022 Knut Sveidqvist

### Moment.js

Verzia `2.29.4`
MIT License
Copyright (c) JS Foundation and other contributors

### pdf.js

Apache License 2.0

### PixiJS

MIT License
Copyright (c) 2013-2017 Mathew Groves, Chad Engler

### Prism

Verzia `1.29.0`
MIT License
Copyright (c) 2012 Lea Verou

### rbush

MIT License
Copyright (c) 2016 Vladimir Agafonkin

### remark

MIT License
Copyright (c) 2014-2020 Titus Wormer <tituswormer@gmail.com>
Copyright (c) 2011-2014, Christopher Jeffrey (https://github.com/chjj/)

### reveal.js

Verzia `4.3.1`
The MIT License
Copyright (C) 2011-2022 Hakim El Hattab, http://hakim.se, and reveal.js contributors

### scrypt

Apache License 2.0

### Turndown

MIT License
Copyright (c) 2017 Dom Christie

### Webpack

MIT License
Copyright JS Foundation and other contributors

### YAML

Verzia `2.7.0`
ISC License
Copyright Eemeli Aro <eemeli@gmail.com>

## Predchádzajúce logo

Logo Obsidian používané pred verziou v1.3.5 bolo adaptované z:

[Crystal shard icon](https://icon-icons.com/icon/Crystal-Shard/88819)
Autor [Chanut is Industries](https://icon-icons.com/users/W52nHhY3W1VlvwyJTwS4d/icon-sets/ "Designer")
Licencia: Attribution 3.0 Unported (CC BY 3.0)
