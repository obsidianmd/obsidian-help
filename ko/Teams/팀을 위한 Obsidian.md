---
permalink: teams
cssclasses:
  - list-cards
  - list-cards-mobile-full
---
Obsidian은 개인용 노트 필기 앱으로 사용할 수 있을 뿐만 아니라, 팀을 위한 협업 도구로도 활용할 수 있습니다.

## 팀과 함께 Obsidian 사용하기

- [[상용 라이선스]]
	- 상용 라이선스를 구매하여 Obsidian을 지원하세요.
- [[팀 전체에 Obsidian 배포|Obsidian 배포]]
	- 팀 전체에 Obsidian을 설치, 업데이트 및 커스터마이즈하는 방법.
- [[팀을 위한 동기화]]
	- 팀 전체의 데이터를 동기화하는 방법.
- [[팀을 위한 게재]]
	- 팀 보관함에서 데이터를 게재하는 방법.

## 중요 자료

아래는 공식 웹사이트의 관련 페이지로 연결되는 중요한 링크이며, 빠른 접근을 위해 여기에 정리했습니다.

- [제품 구매](https://obsidian.md/pricing)
	- 상용 라이선스, Sync 또는 Publish 구매
- [버그 신고](https://forum.obsidian.md/c/bug-reports/7)
	- 앱 문제 신고
- [새로운 기능 요청](https://forum.obsidian.md/tags/c/feature-requests/8/commercial)
	- 기능 요청 제출
- [라이선스 개요](https://obsidian.md/license)
	- 쉬운 언어로 작성된 라이선스 개요 확인
- [서비스 약관](https://obsidian.md/terms)
	- 최신 서비스 약관 확인
- [개인정보 보호 정책](https://obsidian.md/privacy)
	- 개인정보 보호 정책 확인
