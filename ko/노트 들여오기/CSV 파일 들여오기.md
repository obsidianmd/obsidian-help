...
6. CSV 파일의 위치를 선택해요.
7. **들여오기**를 클릭하여 CSV 파일의 데이터가 [[속성]]이 포함된 노트로 변환되는 방식을 설정해요.
8. **계속**을 클릭하고 들여오기가 완료될 때까지 기다려요.
9. 완료되었어요!

## CSV 필드 들여오기 방식 설정하기

CSV 들여오기의 두 번째 단계에서는 템플릿을 사용하여 데이터가 들여와지는 방식을 선택할 수 있어요.

CSV 파일의 각 열에는 파일 헤더의 이름을 나타내는 `{{column_name}}`이라는 변수가 할당돼요. 이 변수를 사용하여 노트의 제목, 위치, 내용 및 [[속성|속성]]을 정의할 수 있어요.
