...
- `--callout-color`는 배경색을 정의해요. 유효한 CSS 색상이면 모두 사용할 수 있어요. 예를 들어 16진수 코드(`#000000`)나 `rgb()` 값 등이 있어요.
...
> [!tip]- 추가 사용자 지정
> `--callout-color`와 `--callout-icon`에만 제한되지 않아요. 콜아웃은 표준 CSS 선택자와 속성을 지원하며, 테두리 및 제목 스타일링 등을 위한 추가 CSS 변수도 사용할 수 있어요.
> 
> 예를 들어, 콜아웃의 제목을 숨기려면:
> 
> ```css
...
> ```
> 
> 더 많은 옵션은 [콜아웃 CSS 변수](https://docs.obsidian.md/Reference/CSS+variables/Editor/Callout) 전체 목록을 참조하세요.
