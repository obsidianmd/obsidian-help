...
publish: true
mobile: true
description: 여러 기기와 플랫폼에서 Obsidian 노트를 동기화하는 방법입니다.
---
...
다음은 Obsidian 커뮤니티 구성원들이 보고한 일반적으로 사용되는 동기화 방법이에요:
...
수십 가지의 대안 동기화 옵션을 [커뮤니티 플러그인 디렉토리](https://community.obsidian.md/search?type=plugin&categories=syncing)에서 찾을 수 있어요.
...
- **iCloud Drive 활성화**:
    - macOS: **시스템 설정 → Apple ID → iCloud → iCloud Drive**로 이동해요.
...
- **iCloud에서 새 보관함 생성**:
    ...
    - iOS:
        ...
        3. **iCloud에 저장**을 켜요.
        4. **생성**을 탭해요.
- **보관함 폴더를 다운로드 상태로 유지** (선택 사항이지만 로드 시간을 빠르게 유지하기 위해 강력히 권장해요):
    - macOS 15 (Sequoia) 이상: iCloud Drive에서 **Obsidian** 폴더를 우클릭하고 **항상 다운로드 유지**를 선택해요.
    - macOS 14 (Sonoma) 이전: iCloud 설정에서 **Mac 저장 공간 최적화**를 비활성화해요. 이 설정은 Obsidian뿐만 아니라 기기의 모든 iCloud 저장소에 영향을 미쳐요.
