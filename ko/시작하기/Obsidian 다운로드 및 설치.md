...
permalink: install
---
Obsidian은 모든 주요 데스크톱 및 모바일 플랫폼에서 사용할 수 있습니다. 아래는 Obsidian을 다운로드하고 설치하는 모든 지원 방법입니다.
...
4. 다른 앱을 열 때와 동일한 방법으로 Obsidian을 실행합니다.
...
5. 다른 앱을 열 때와 동일한 방법으로 Obsidian을 실행합니다.
...
4. 터미널에서 다음 명령을 실행하여 Snap 패키지를 설치합니다: (`--dangerous` 플래그는 Snap을 만든 Canonical 회사가 패키지를 검토하지 않았기 때문에 필요하며, `--classic` 플래그는 Obsidian이 노트가 저장된 샌드박스 외부에 접근할 수 있도록 합니다)
...
3. 다른 앱을 열 때와 동일한 방법으로 Obsidian을 실행합니다.
