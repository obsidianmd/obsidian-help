...
async function main() {
  ...
  if (missingFolders.length > 0) {
    ...
    for (const [en, fr] of Object.entries(translated)) {
      const badChars = /[\/\\?*:"<>|]/.exec(fr);
      if (badChars) {
        console.warn(`  ⚠ Folder "${en}" → "${fr}" contains "${badChars[0]}" — this character is not allowed in folder names on Windows. Fix this before syncing.`);
      }
  ...
  if (missingFiles.length > 0) {
    ...
    for (let i = 0; i < missingFiles.length; i += BATCH) {
      ...
      for (const [permalink, frName] of Object.entries(translated)) {
        const badChars = /[\/\\?*:"<>|]/.exec(frName);
        if (badChars) {
          console.warn(`  ⚠ File [${permalink}] → "${frName}" contains "${badChars[0]}" — this character is not allowed in file names on Windows. Fix this in filenames.txt before syncing.`);
        }
