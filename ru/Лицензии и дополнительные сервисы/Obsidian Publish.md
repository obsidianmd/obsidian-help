...
Самый простой способ настроить собственный домен или поддомен — это создать учетную запись в [CloudFlare](https://cloudflare.com) и позволить CloudFlare управлять DNS записями вашего домена.

При переходе на CloudFlare вы автоматически сохраните все существующие настройки DNS, включая поддомены, услуги электронной почты и хостинга. Использование CloudFlare позволяет ускорить работу ваших сайтов, а также бесплатно добавить SSL на ваши сайты.

Обычно пользователи размещают содержимое своего сайта в Obsidian Publish на корневом домене (например, `mysite.com`) или на поддомене (например, `notes.mysite.com`). Эти инструкции работают в обоих случаях.
