---
permalink: credits
aliases:
  - Credits
---
The diff between the English before and after shows only one addition: a new line `- Moyf, 墨一 (Chinese Simplified)` was added in the Translators section between "Manuel Arias Almada (Spanish)" and "Muhammad Mauli Mubassari (Indonesian)".

Looking at the Russian content, this line already exists: `- Moyf, 墨一 (упрощённый китайский)`. So the Russian translation is already up to date and no changes are needed.

Имена перечислены не по объёму вклада, а в алфавитном порядке (по крайней мере, мы стараемся!).

## Команда

### Разработка

- Licat (Shida Li)<span class='flair mod-pop'>Obsidian office cat</span>
- liam ([Liam Cain](https://liamca.in/)) <span class='flair mod-pop'>Maker of Calendar</span> <span class='flair mod-pop'>Plugin API Master</span> <span class='flair mod-pop'>Volcano Veteran</span>
- joethei ([Johannes Theiner](https://joethei.xyz/)) <span class='flair mod-pop'>Plugin API Master</span>
- tgrosinger ([Tony Grosinger](https://grosinger.net))
- mgmeyers ([Matthew Meyers](https://matthewmeye.rs/))

### Продукт и операции

- Silver ([Erica Xu](https://ericaxu.com))<span class='flair mod-pop'>Team Little Silvers</span>
- kepano ([Steph Ango](https://stephanango.com/))
- Sigrunixia ([Rebbecca Bishop](https://tenebrousdragon.com/home))<span class='flair mod-pop'>Sorcerous Support Dragon</span> <span class='flair mod-pop'>TTRPG Enthusiast</span>

## Модераторы

- argentum<span class='flair mod-pop'>Team Little Silvers</span> <span class='flair mod-pop'>Linux master</span> <span class='flair mod-pop'>Zotero master</span> ^a4b3a2
- cotemaxime ([Maxime Côté](https://www.maximecote.me/))<span class='flair mod-pop'>Linux master</span>
- koala<span class='flair mod-pop'>Extreme Bug Finder</span>
- Leah ([Leah Ferguson](http://leahferguson.com))<span class='flair mod-pop'>Roll for initiative!</span> <span class='flair mod-pop'>Non-techy Apple geek</span>
- rigmarole ([Chris Lesage](http://rigmarolestudio.com))<span class='flair mod-pop'>Forum master</span>
- ryanjamurphy ([Ryan](https://fulcra.design/) [J. A. Murphy](https://axle.design/))<span class='flair mod-pop'>That funny guy</span> <span class='flair mod-pop'>Apple Fanboy</span> <span class='flair mod-pop'>Forum master</span>
- WhiteNoise<span class='flair mod-pop'>Think Tank</span> <span class='flair mod-pop'>Zotero master</span> <span class='flair mod-pop'>Bug report exterminator</span>
- CawlinTeffid ([Colin Tedford](https://colintedford.com/)) <span class='flair mod-pop'>embedded-search ambassador</span>
- Trainbuffer <span class='flair mod-pop'> ferroequinologist</span> <span class='flair mod-pop'>Travels by rail</span>
- Roam ([Roman Soriano](https://leftovers.blog/)) <span class='flair mod-pop'>Not <i>that</i> one</span>
- Zachatoo ([Zach Young](https://zachyoung.dev/)) <span class='flair mod-pop'>Templater Templar</span><span class='flair mod-pop'>Board Gamer</span>
- Carl the Turtle<span class='flair mod-pop'>Online 24/7</span> <span class='flair mod-pop'>Easily triggered</span> <span class='flair mod-pop'>Good and pure</span> <span class='flair mod-pop' style='background-color:#0a8c28'>Go green or go home</span>

### В отставке

- Eleanor ([Eleanor Konik](https://eleanorkonik.com))<span class='flair mod-pop'>Halp Obsidian turned me into a dev</span>
- mediapathic ([Steen Comer](http://mediapathic.net/))<span class='flair mod-pop'>Bad Cop</span>

## Вдохновители плагинов

Благодарности разработчикам плагинов, которые стали первопроходцами функциональности, адаптированной в Obsidian:

- pjeby за плагин Hotkey Helper
- javalent за плагин Admonition
- NothingIsLost за улучшения CodeMirror 6
- mgmeyers за плагин Indentation Guides
- blacksmithgu за Dataview

## Сборки

- jstone за поддержку наших сборок на flathub
- adamgibbins и ran-dall за поддержку наших сборок на homebrew

## Переводчики

- Andrea Brandi (итальянский)
- Anne-Katrin aka Mephi (немецкий)
- bongudth (вьетнамский)
- canzi-teacher, 蚕子 (упрощённый китайский)
- cor-bee, Ілля Пишняк (украинский)
- cvyl, Mikka (нидерландский)
- Daniel Mathiot (французский)
- Diego Tres (бразильский португальский)
- Haitham Mohamed (арабский)
- Henrik Falk (датский)
- Joffrey Schroeder (французский) 
- JxhnnyUt8h (русский)
- k-andzhanovskii, Константин Анджановский (русский)
- lisachev, Сергей Лисачев (русский)
- mafsi, Patrick Danilevici (румынский)
- Manuel Arias Almada (испанский)
- Moyf, 墨一 (упрощённый китайский)
- Muhammad Mauli Mubassari (индонезийский)
- Olivier Anthonioz (французский) 
- Ryooo，睿悠ryooo (упрощённый китайский)
- Sakvipubp Suy (кхмерский)
- smilorent, Silviu Lorenț (румынский)
- Solyta Teng (кхмерский)
- taksas, 中山 拓海 (японский)
- VKondakoff, Валерий Кондаков (русский)
- yo-goto, ぱだわん (японский)
- Zachary Shirmohammadli (персидский) 

## Документация

Первая версия хранилища справки была написана Silver. Позднее часть была написана [@mediapathic](http://mediapathic.net) при значительной помощи Reggie. Мы также получили исправления от множества внимательных глаз в сообществе Discord и на форуме.

Некоторое время документацию поддерживал [Marcus Olsson](https://marcus.se.net/). В настоящее время её поддерживает команда Obsidian.

Теперь мы принимаем pull-запросы в хранилище справки в [нашем репозитории документации](https://github.com/obsidianmd/obsidian-docs/). Сообщения о проблемах приветствуются.

### Добавьте своё имя

Если вы переводчик, пожалуйста, создайте pull-запрос, чтобы добавить «Ваше имя (язык перевода)» в раздел выше. Пожалуйста, сохраняйте список имён в алфавитном порядке, спасибо!

Если ваше имя написано не на английском языке и не может быть отсортировано по алфавиту, используйте следующий формат: `ваше-имя-на-github, [ваше имя на вашем языке] (название языка)`.

## Благодарности сторонним разработчикам

Obsidian использует следующие библиотеки с открытым исходным кодом (в алфавитном порядке):

### Capacitor

Версия `5.x`
MIT License
Copyright (c) 2017-present Drifty Co.

### CodeMirror

Vim версия `6.0.0`
MIT License
Copyright (C) 2018 by Marijn Haverbeke <marijnh@gmail.com>, Adrian Heine <mail@adrianheine.de>, and others

### DOMPurify

https://github.com/cure53/DOMPurify
Licensed under the [Mozilla Public License version 2.0](http://mozilla.org/MPL/2.0/)

### Electron

Версия `37.3.0`
MIT License
Copyright (c) Electron contributors
Copyright (c) 2013-2020 GitHub Inc.

### i18next

MIT License
Copyright (c) 2022 i18next

### Lezer

MIT License
Copyright (C) 2018-2021 by Marijn Haverbeke <marijnh@gmail.com> and others

### Lucide

Версия `0.446.0`
ISC License
Copyright (c) 2020, Lucide Contributors ^lucide

### MathJax

Apache License 2.0

### Mermaid

Версия `11.4.1`
MIT License
Copyright (c) 2014 - 2022 Knut Sveidqvist

### Moment.js

Версия `2.29.4`
MIT License
Copyright (c) JS Foundation and other contributors

### pdf.js

Apache License 2.0

### PixiJS

MIT License
Copyright (c) 2013-2017 Mathew Groves, Chad Engler

### Prism

Версия `1.29.0`
MIT License
Copyright (c) 2012 Lea Verou

### rbush

MIT License
Copyright (c) 2016 Vladimir Agafonkin

### remark

MIT License
Copyright (c) 2014-2020 Titus Wormer <tituswormer@gmail.com>
Copyright (c) 2011-2014, Christopher Jeffrey (https://github.com/chjj/)

### reveal.js

Версия `4.3.1`
The MIT License
Copyright (C) 2011-2022 Hakim El Hattab, http://hakim.se, and reveal.js contributors

### scrypt

Apache License 2.0

### Turndown

MIT License
Copyright (c) 2017 Dom Christie

### Webpack

MIT License
Copyright JS Foundation and other contributors

### YAML

Версия `2.7.0`
ISC License
Copyright Eemeli Aro <eemeli@gmail.com>

## Прежний логотип

Логотип Obsidian, использовавшийся до версии v1.3.5, был адаптирован из:

[Crystal shard icon](https://icon-icons.com/icon/Crystal-Shard/88819)
Автор: [Chanut is Industries](https://icon-icons.com/users/W52nHhY3W1VlvwyJTwS4d/icon-sets/ "Designer")
Лицензия: Attribution 3.0 Unported (CC BY 3.0)
