...
1. **Аккаунт Notion (рекомендуется)** подключается напрямую к вашему рабочему пространству и позволяет сохранить базы данных и формулы, которые конвертируются в [[Введение в Базы|Базы]]. Требует токен интеграции Notion и подключение к интернету.
2. **Импорт из файла** использует файлы экспорта `.zip` из Notion. Не сохраняет базы данных, но не требует API-токена или подключения к интернету.

## Импорт из аккаунта Notion

### Создание токена доступа к API Notion

Для доступа к данным Notion через API необходим токен доступа. Этот шаг занимает около двух минут.

Токен — это длинная строка из цифр и букв, обычно начинающаяся с `ntn_...`, которая позволит загрузить ваши данные из Notion.

1. Войдите в панель управления [Notion Connections](https://app.notion.com/developers/connections).
2. Выберите **New connection**.
	1. Дайте соединению имя, например «Personal». Можно использовать любое имя.
	2. Выберите **Access token** в качестве метода аутентификации.
	3. Выберите рабочее пространство, которое хотите экспортировать, в поле **Installable in**.

![[notion-integration.png#interface]]

3. Нажмите **Create connection**.
4. На вкладке **Configuration** ваш API-токен доступен в поле **Access token**.
5. Нажмите **Show**, затем **Copy**.
6. Сохраните токен в безопасном месте, например в менеджере паролей.

![[notion-token.png#interface]]

Далее предоставьте соединению доступ к страницам и базам данных Notion, которые вы хотите импортировать.

1. Перейдите на вкладку **Content access** созданного соединения.
2. Добавьте страницы и базы данных, которые хотите импортировать.
...
6. В поле **API token** нажмите **Link...**, чтобы добавить новый секрет. В поле **ID** укажите имя, например `notion`, а в поле **Secret** вставьте ваш **Access token** из Notion.
...
1. Перейдите в **Settings** в верхней части боковой панели Notion.
