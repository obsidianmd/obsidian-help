---
permalink: credits
aliases:
  - Credits
---
Namen worden niet op hoeveelheid bijdrage vermeld, maar alfabetisch (althans, dat proberen we!).

## Kernteam

Het kernteam dat Obsidian bouwt en onderhoudt.

- codedmart (Brandon Martin)
- dharam (Dharam Kapila)
- kepano ([Steph Ango](https://stephanango.com/))
- Licat (Shida Li)
- liam ([Liam Cain](https://liamca.in/)) <span class='flair'>Maker of Calendar</span> <span class='flair'>Plugin API Master</span> <span class='flair'>Volcano Veteran</span>
- Silver ([Erica Xu](https://ericaxu.com))<span class='flair'>Team Little Silvers</span>
- Sigrunixia ([Rebbecca Bishop](https://tenebrousdragon.com/home))<span class='flair'>Sorcerous Support Dragon</span> <span class='flair'>TTRPG Enthusiast</span>
- tgrosinger ([Tony Grosinger](https://grosinger.net))

## Bijdragers

Deeltijdontwikkelaars die helpen Obsidian te bouwen.

- cmptr (Aaron Bell)
- mgmeyers ([Matthew Meyers](https://matthewmeye.rs/))

## Community-review

Bijdragers aan het [Community](https://community.obsidian.md/) plug-in/thema-reviewproces en -tools.

- Fevol
- mnaoumov ([Michael Naumov](https://mnaoumov.dev/))
- Saberzero1 (Emile Bangma)
- Zachatoo ([Zach Young](https://zachyoung.dev/)) <span class='flair'>Templater Templar</span><span class='flair'>Board Gamer</span>

## Moderatie

Moderators beheren het Obsidian Forum, Discord en Reddit.

- argentum<span class='flair'>Team Little Silvers</span> <span class='flair'>Linux master</span> <span class='flair'>Zotero master</span> ^a4b3a2
- CawlinTeffid ([Colin Tedford](https://colintedford.com/)) <span class='flair'>embedded-search ambassador</span>
- cotemaxime ([Maxime Côté](https://www.maximecote.me/))<span class='flair'>Linux master</span>
- koala<span class='flair'>Extreme Bug Finder</span>
- Leah ([Leah Ferguson](http://leahferguson.com))<span class='flair'>Roll for initiative!</span> <span class='flair'>Non-techy Apple geek</span>
- rigmarole ([Chris Lesage](http://rigmarolestudio.com))<span class='flair'>Forum master</span>
- Roam ([Roman Soriano](https://leftovers.blog/)) <span class='flair'>Not <i>that</i> one</span>
- Trainbuffer <span class='flair'> ferroequinologist</span> <span class='flair'>Travels by rail</span>
- WhiteNoise<span class='flair'>Think Tank</span> <span class='flair'>Zotero master</span> <span class='flair'>Bug report exterminator</span>
- Carl the Turtle<span class='flair'>Online 24/7</span> <span class='flair'>Easily triggered</span> <span class='flair'>Good and pure</span> 

## Alumni

Voormalige moderators en teamleden die aan nieuwe avonturen zijn begonnen!

- Eleanor ([Eleanor Konik](https://eleanorkonik.com))<span class='flair'>Halp Obsidian turned me into a dev</span>
- joethei ([Johannes Theiner](https://joethei.xyz/)) <span class='flair'>Plugin API Master</span>
- mediapathic ([Steen Comer](http://mediapathic.net/))<span class='flair'>Bad Cop</span>
- ryanjamurphy ([Ryan](https://fulcra.design/) [J. A. Murphy](https://axle.design/))<span class='flair'>That funny guy</span> <span class='flair'>Apple Fanboy</span> <span class='flair'>Forum master</span>

## Plug-in-inspiraties

Met dank aan deze plug-in-ontwikkelaars die functionaliteit hebben gepionierd die in Obsidian is overgenomen:

- pjeby voor de Hotkey Helper-plug-in
- javalent voor de Admonition-plug-in
- javalent voor de Settings Search-plug-in
- NothingIsLost voor CodeMirror 6-verbeteringen
- mgmeyers voor de Indentation Guides-plug-in
- blacksmithgu voor Dataview

## Builds

- jstone voor het onderhouden van onze builds op Flathub
- adamgibbins en ran-dall voor het onderhouden van onze builds op homebrew

## Vertalers

Zie ook de volledige lijst van [vertaalbijdragers](https://github.com/obsidianmd/obsidian-translations/graphs/contributors?from=7%2F31%2F2020).

- Andrea Brandi (Italiaans)
- Anne-Katrin aka Mephi (Duits)
- bongudth (Vietnamees)
- canzi-teacher, 蚕子 (Vereenvoudigd Chinees)
- cor-bee, Ілля Пишняк (Oekraïens)
- cvyl, Mikka (Nederlands)
- Daniel Mathiot (Frans)
- Diego Tres (Braziliaans Portugees)
- Haitham Mohamed (Arabisch)
- Henrik Falk (Deens)
- Joffrey Schroeder (Frans) 
- JxhnnyUt8h (Russisch)
- k-andzhanovskii, Константин Анджановский (Russisch)
- lisachev, Сергей Лисачев (Russisch)
- mafsi, Patrick Danilevici (Roemeens)
- Manuel Arias Almada (Spaans)
- Moyf, 墨一 (Vereenvoudigd Chinees)
- Muhammad Mauli Mubassari (Indonesisch)
- Olivier Anthonioz (Frans) 
- Ryooo，睿悠ryooo (Vereenvoudigd Chinees)
- Sakvipubp Suy (Khmer)
- smilorent, Silviu Lorenț (Roemeens)
- Solyta Teng (Khmer)
- taksas, 中山 拓海 (Japans)
- VKondakoff, Валерий Кондаков (Russisch)
- yo-goto, ぱだわん (Japans)
- Zachary Shirmohammadli (Perzisch) 

## Ondersteunende organisaties

Organisaties die Obsidian ondersteunen met een [[Commerciële licentie]] worden vermeld op onze [Enterprise](https://obsidian.md/enterprise/)-pagina. 

## Documentatie

De eerste versie van de help-kluis is geschreven door Silver. Later is een deel ervan geschreven door [@mediapathic](http://mediapathic.net), met grote hulp van Reggie. We hebben ook correcties ontvangen van de vele oplettende ogen in de Discord- en forumcommunity.

De documentatie werd een tijdlang onderhouden door [Marcus Olsson](https://marcus.se.net/). Momenteel wordt deze onderhouden door het Obsidian-team.

We accepteren nu pull requests voor de help-kluis in [onze docs-repo](https://github.com/obsidianmd/obsidian-docs/). Issues zijn welkom.

### Voeg je naam toe

Als je vertaler bent, maak dan een pull request aan om "Je Naam (bijgedragen taal)" toe te voegen aan de bovenstaande sectie. Houd de lijst met namen alsjeblieft alfabetisch, bedankt!

Als je naam niet in het Engels is en niet alfabetisch kan worden gerangschikt, gebruik dan dit formaat: `je-github-gebruikersnaam, [je naam in je taal] (Taalnaam)`.

## Erkenningen van derden

Obsidian maakt gebruik van de onderstaande open source-bibliotheken (in alfabetische volgorde):

### Capacitor

Versie `5.x`
MIT License
Copyright (c) 2017-present Drifty Co.

### CodeMirror

Vim Versie `6.0.0`
MIT License
Copyright (C) 2018 by Marijn Haverbeke <marijnh@gmail.com>, Adrian Heine <mail@adrianheine.de>, and others

### DOMPurify

https://github.com/cure53/DOMPurify
Licensed under the [Mozilla Public License version 2.0](http://mozilla.org/MPL/2.0/)

### Electron

Versie `37.3.0`
MIT License
Copyright (c) Electron contributors
Copyright (c) 2013-2020 GitHub Inc.

### i18next

MIT License
Copyright (c) 2022 i18next

### Lezer

MIT License
Copyright (C) 2018-2021 by Marijn Haverbeke <marijnh@gmail.com> and others

### Lucide

Versie `0.446.0`
ISC License
Copyright (c) 2020, Lucide Contributors ^lucide

### MathJax

Apache License 2.0

### Mermaid

Versie `11.4.1`
MIT License
Copyright (c) 2014 - 2022 Knut Sveidqvist

### Moment.js

Versie `2.29.4`
MIT License
Copyright (c) JS Foundation and other contributors

### pdf.js

Apache License 2.0

### PixiJS

MIT License
Copyright (c) 2013-2017 Mathew Groves, Chad Engler

### Prism

Versie `1.29.0`
MIT License
Copyright (c) 2012 Lea Verou

### rbush

MIT License
Copyright (c) 2016 Vladimir Agafonkin

### remark

MIT License
Copyright (c) 2014-2020 Titus Wormer <tituswormer@gmail.com>
Copyright (c) 2011-2014, Christopher Jeffrey (https://github.com/chjj/)

### reveal.js

Versie `4.3.1`
The MIT License
Copyright (C) 2011-2022 Hakim El Hattab, http://hakim.se, and reveal.js contributors

### scrypt

Apache License 2.0

### Turndown

MIT License
Copyright (c) 2017 Dom Christie

### Webpack

MIT License
Copyright JS Foundation and other contributors

### YAML

Versie `2.7.0`
ISC License
Copyright Eemeli Aro <eemeli@gmail.com>

## Vorig logo

Het Obsidian-logo dat vóór v1.3.5 werd gebruikt, is gebaseerd op:

[Crystal shard icon](https://icon-icons.com/icon/Crystal-Shard/88819)
Door [Chanut is Industries](https://icon-icons.com/users/W52nHhY3W1VlvwyJTwS4d/icon-sets/ "Designer")
Licentie: Attribution 3.0 Unported (CC BY 3.0)
