---
permalink: import
cssclasses:
  - list-cards
---
Apps komen en gaan, maar je gegevens zouden moeten blijven. Obsidian gebruikt niet-proprietaire platte tekst Markdown-bestanden die lokaal op je apparaat worden opgeslagen. Je hebt volledige controle over je gegevens, wat betekent dat je Obsidian offline kunt gebruiken en eenvoudig kunt overstappen naar een andere app als dat ooit nodig is.

## Importeren vanuit andere apps en bestandsformaten

- <span class="icon-app icon-notion"></span> [[Importeren vanuit Notion|Notion]]
- <span class="icon-app icon-airtable"></span> [[Importeren vanuit Airtable|Airtable]]
- <span class="icon-app icon-onenote"></span> [[Importeren vanuit Microsoft OneNote|Microsoft OneNote]]
- <span class="icon-app icon-evernote"></span> [[Importeren vanuit Evernote|Evernote]]
- <span class="icon-app icon-apple-notes"></span> [[Importeren vanuit Apple Notities|Apple Notes]]
- <span class="icon-app icon-apple-journal"></span> [[Importeren vanuit Apple Journal|Apple Journal]]
- <span class="icon-app icon-google-keep"></span> [[Importeren vanuit Google Keep|Google Keep]]
- <span class="icon-app icon-bear"></span> [[Importeren vanuit Bear|Bear]]
- <span class="icon-app icon-craft"></span> [[Importeren vanuit Craft|Craft]]
- <span class="icon-app icon-roam"></span> [[Importeren vanuit Roam Research|Roam Research]]
- <span class="icon-app icon-logseq"></span> [[Importeren vanuit Logseq|Logseq]]
- <span class="icon-app icon-md"></span> [[Importeren vanuit Tomboy en Gnote|Tomboy en Gnote]]
- <span class="icon-app icon-html"></span> [[HTML-bestanden importeren|HTML-bestanden]]
- <span class="icon-app icon-md"></span> [[CSV-bestanden importeren]]
- <span class="icon-app icon-md"></span> [[Markdown-bestanden importeren|Markdown-bestanden]]
- <span class="icon-app icon-md"></span> [[Textbundle-bestanden importeren|Textbundle-bestanden]]

## Meer formaten

Zie je je app niet? Je kunt een verzoek indienen of bijdragen aan de [open-source repository](https://github.com/obsidianmd/obsidian-importer) voor de [[Importeren]]-plug-in.

Er zijn aanvullende handleidingen gedeeld door onze community die je kunnen helpen bij het migreren naar Obsidian:

- [Day One](https://github.com/obsidianmd/obsidian-importer/issues/55)
- [Diaro](https://github.com/obsidianmd/obsidian-importer/issues/38)
- [Remnote](https://forum.obsidian.md/t/can-anybody-help-with-migrating-remnote-to-obsidian/40156/2)
- [Samsung Notes](https://github.com/obsidianmd/obsidian-importer/issues/307)
- [TiddlyWiki](https://forum.obsidian.md/t/import-from-tiddlywiki-5-to-obsidian/731)
- [TheBrain](https://github.com/obsidianmd/obsidian-importer/issues/97)
- [Ulysses](https://github.com/obsidianmd/obsidian-importer/issues/18)
- [Zim](https://github.com/obsidianmd/obsidian-importer/issues/39)
- [zkn3](https://forum.obsidian.md/t/migrating-from-zkn3-to-obsidian-without-losing-your-tags-and-internal-links-documentation/7457)
