...
3. Créez la note en cliquant sur le lien tout en appuyant sur `Ctrl` (ou `Cmd` sur macOS).

## Naviguer entre les notes

Comme vous l'avez vu dans les étapes précédentes, vous pouvez cliquer sur un lien tout en appuyant sur `Ctrl` (ou `Cmd` sur macOS) pour accéder à la note liée.

Une autre façon de naviguer entre les notes est d'utiliser les _liens retour_. Un lien retour vous permet de naviguer dans la direction opposée d'un lien existant.

1. Ouvrez la note « Isaac Newton ».
2. Dans la barre latérale droite, cliquez sur l'onglet **Liens retour**.
