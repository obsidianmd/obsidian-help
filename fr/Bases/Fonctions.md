...
- Échappe les caractères spéciaux dans une chaîne de caractères pour la rendre sûre lors de l'inclusion dans du HTML.
...
- Convertit une chaîne de caractères en un extrait de code qui s'affiche en tant que HTML.
...
- Retourne une valeur qui s'affiche en tant qu'icône dans une vue. Le nom de l'icône doit correspondre à une icône Lucide prise en charge.
...
- Retourne true si la valeur est du type fourni.
- Exemple : `"example".isType("string")` et `true.isType("boolean")` retournent tous deux true.
...
- Retourne true si la chaîne contient `value`.
...
- Retourne true si la chaîne contient toutes les `values`.
...
- `values` sont une ou plusieurs sous-chaînes à rechercher.
- Retourne true si la chaîne contient au moins une des `values`.
...
- Retourne true si cette chaîne se termine par `query`.
...
- Retourne true si la chaîne ne contient aucun caractère, ou n'est pas présente.
...
- Retourne une sous-chaîne de `start` (inclus) à `end` (exclus).
...
- Retourne true si cette chaîne commence par `query`.
...
- Retourne true si le nombre n'est pas présent.
...
- Retourne true si la liste contient `value`.
...
- Retourne true si la liste contient toutes les `values`.
...
- `values` sont un ou plusieurs éléments à rechercher.
- Retourne true si la liste contient au moins une des `values`.
...
- `start` est l'index de début inclus.
- `end` est l'index de fin exclusif optionnel.
- Retourne une copie superficielle d'une portion de la liste de `start` (inclus) à `end` (exclus).
...
- Retourne true si `file` possède un lien vers `otherFile`.
...
- Retourne true si le fichier possède l'un des mots-clés dans `values`.
- Exemple : `file.hasTag("tag1", "tag2")` retourne `true` si le fichier possède le mot-clé `#tag1` ou `#tag2`. Cela inclut également les [[Mots-clés#Mots-clés imbriqués|mots-clés imbriqués]], tels que `#tag1/a` ou `#tag2/b`.
...
- Retourne true si l'objet ne possède aucune propriété propre.
- Exemple : `{}.isEmpty()` retourne `true`.
...
- Retourne true si l'expression régulière correspond à `value`.
