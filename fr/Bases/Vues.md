...
La première vue de votre liste de vues se chargera par défaut. Glissez les vues par leur icône pour modifier leur ordre.
...
- **N'importe laquelle des conditions suivantes est vraie** est une instruction `ou` — les résultats seront affichés si *n'importe laquelle* des conditions du groupe de filtres est remplie.
- **Aucune des conditions suivantes n'est vraie** est une instruction `non` — les résultats ne seront pas affichés si *n'importe laquelle* des conditions du groupe de filtres est remplie.

#### Groupes de filtres

Les groupes de filtres vous permettent de créer une logique plus complexe en combinant des conjonctions.

#### Éditeur de filtres avancé

Cliquez sur le bouton code ![[lucide-code-xml.svg#icon]] pour utiliser l'éditeur de **filtres avancés**. Celui-ci affiche la [[Syntaxe des Bases|syntaxe]] brute du filtre et peut être utilisé avec des [[Fonctions|fonctions]] plus complexes qui ne peuvent pas être représentées via l'interface pointer-cliquer.
...
- **Texte** : trier *alphabétiquement* (A→Z) ou en *ordre alphabétique inverse* (Z→A).
- **Nombre** : trier du *plus petit au plus grand* (0→1) ou du *plus grand au plus petit* (1→0).
- **Date et heure** : trier de l'*ancien au récent* ou du *récent à l'ancien*.
...
Le menu *résultats* affiche le nombre de résultats dans la vue. Cliquez sur le bouton des résultats pour limiter le nombre de résultats et accéder à des actions supplémentaires.
