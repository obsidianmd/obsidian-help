...
Il existe deux possibilités pour appliquer des filtres :

1. Au niveau global `filters` (comme montré ci-dessus) où ils s'appliquent à toutes les vues de la base.
...
La section `filters` contient soit une instruction de filtre unique sous forme de chaîne de caractères, soit un objet filtre défini de manière récursive. Les objets filtres peuvent contenir `and`, `or` ou `not`. Ces clés sont une liste hétérogène d'autres objets filtres ou d'instructions de filtre sous forme de chaînes. Une instruction de filtre est une ligne qui s'évalue comme vraie ou fausse lorsqu'elle est appliquée à une note. Elle peut être l'une des suivantes :

- Une comparaison de base utilisant les opérateurs arithmétiques standard.
- Une fonction. De nombreuses [[Fonctions]] sont intégrées, et les modules peuvent en ajouter d'autres.

La syntaxe et les fonctions disponibles pour les filtres et les formules sont identiques.
...
Les propriétés de formule prennent en charge les opérateurs arithmétiques de base et de nombreuses [[Fonctions]] intégrées. À l'avenir, les modules pourront ajouter des fonctions utilisables dans les formules.
...
- Les **propriétés de note** sont des propriétés définies dans les métadonnées de la note. Par exemple `note.price` ou `note["price"]`.
  Si aucun préfixe n'est spécifié, la propriété est considérée comme une propriété de `note`.
...
Dans cet exemple, la formule `customAverage` est identique au résumé `Average` par défaut, sauf que la valeur est arrondie à un nombre de décimales différent. Dans les formules de résumé, le mot-clé `values` est une liste contenant toutes les valeurs de cette propriété pour chaque note dans l'ensemble de résultats. La formule de résumé doit retourner une seule `Value`.

Notez que cette section `summaries` est différente de la section `summaries` dans la configuration de la vue (expliquée ci-dessous) où les formules de résumé sont attribuées à des propriétés spécifiques.

#### Formules de résumé par défaut

| Nom       | Type d'entrée | Description                                                          |
| --------- | ------------- | -------------------------------------------------------------------- |
| Average   | Number        | La moyenne mathématique de tous les nombres des valeurs en entrée.   |
| Min       | Number        | Le plus petit nombre des valeurs en entrée.                          |
| Max       | Number        | Le plus grand nombre des valeurs en entrée.                          |
| Sum       | Number        | La somme de tous les nombres en entrée.                              |
| Range     | Number        | La différence entre `Max` et `Min`.                                  |
| Median    | Number        | La médiane mathématique de tous les nombres des valeurs en entrée.   |
| Stddev    | Number        | L'écart-type de tous les nombres des valeurs en entrée.              |
| Earliest  | Date          | La date la plus ancienne des valeurs en entrée.                      |
| Latest    | Date          | La date la plus récente des valeurs en entrée.                       |
...
| Empty     | Any           | Le nombre de valeurs vides en entrée.                                |
| Filled    | Any           | Le nombre de valeurs non vides en entrée.                            |
| Unique    | Any           | Le nombre de valeurs uniques en entrée.                              |

### Vues

La section `views` définit comment les données peuvent être rendues. Chaque entrée dans la liste `views` définit une vue séparée des mêmes données, et il peut y avoir autant de vues différentes que nécessaire.
...
À l'avenir, l'API permettra aux vues de lire et écrire ces valeurs, permettant à la vue de construire sa propre interface de configuration.
...
Les [[Propriétés|propriétés de note]] ne sont disponibles que pour les fichiers Markdown et sont stockées dans les métadonnées YAML de chaque note. Ces propriétés sont accessibles au format `note.author` ou simplement `author` comme raccourci.
...
Par exemple, un filtre `file.ext == "md"` sera vrai pour tous les fichiers Markdown et faux dans le cas contraire.

| Propriété      | Type   | Description                                                   |
| ------------- | ------ | ------------------------------------------------------------- |
| `file.backlinks`  | List   | Liste des fichiers de liens retour. Note : Cette propriété est coûteuse en performance. Quand c'est possible, inversez la recherche et utilisez `file.links`. Ne rafraîchit pas automatiquement les résultats lorsque le coffre est modifié. |
...
Lorsque la base est intégrée dans un autre fichier, `this` pointe vers les propriétés du fichier _d'intégration_ (la note ou le Canvas qui contient la base). Par exemple, utiliser `this.file.name` retourne le nom du fichier d'intégration, pas de la base.
...
- `now() + "1 day"` retourne une date-heure exactement 24 heures après le moment de l'exécution.
- `file.mtime > now() - "1 week"` retourne `true` si le fichier a été modifié dans la dernière semaine.
...
Les bases possèdent un système de types qui est utilisé par les formules et les filtres pour appliquer des fonctions aux propriétés.

### Chaînes de caractères, nombres et booléens

Les chaînes de caractères, les nombres et les booléens sont des valeurs « primitives » qui ne nécessitent pas de fonction pour être créées.

- Les chaînes de caractères sont encadrées par des guillemets simples ou doubles, par exemple `"message"`.
- Les nombres sont écrits sous forme de chiffres et peuvent optionnellement être encadrés de parenthèses pour plus de clarté. Par exemple, `1` ou `(2.5)`.
- Les booléens sont écrits `true` ou `false` sans guillemets.

### Dates et durées

Les dates représentent une date spécifique, ou une date et heure selon la fonction utilisée pour les créer, ou le type qui a été attribué à la [[Propriétés|propriété]].
...
- Comparez les dates en utilisant les opérateurs de comparaison (par ex. `>` ou `<`) et les opérateurs arithmétiques (par exemple, `(now() + "1d") - now()` retourne `86400000` millisecondes.)
...
- Transformez un élément unique en liste en utilisant la fonction `list()`. Ceci est particulièrement utile pour les propriétés qui peuvent contenir un mélange de listes ou de valeurs uniques.
- Accédez aux éléments de liste en utilisant des crochets et un index commençant à 0. Par exemple, `property[0]` retourne le premier élément de la liste.
- Accédez aux éléments d'objet en utilisant des crochets et le nom de l'élément ou la notation par point. Par exemple, `property.subprop` ou `property["subprop"]`.

### Fichiers et liens

Les [[Lier des notes|liens wiki]] dans les [[Propriétés|propriétés des métadonnées]] sont automatiquement reconnus comme des objets Lien. Les liens s'affichent sous forme de lien cliquable dans la [[Vues|vue]].

- Pour construire un lien, utilisez la [[Fonctions|fonction]] globale `link`, par exemple `link("filename")` ou `link("https://obsidian.md")`.
- Vous pouvez créer un lien à partir de n'importe quelle chaîne de caractères, par exemple, `link(file.ctime.date().toString())`.
...
Un objet File peut être transformé en lien en utilisant `file.asLink()` avec un texte d'affichage optionnel.
...
Les liens peuvent être comparés à des fichiers tels que `file` ou `this`. Ils seront égaux si le lien résout vers le fichier. Par exemple, `author == this`.

Les liens peuvent également être vérifiés dans une recherche de contenu de liste, par exemple, `authors.contains(this)`.
