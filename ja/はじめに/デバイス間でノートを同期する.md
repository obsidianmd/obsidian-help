...
Obsidianコミュニティのメンバーから報告されている一般的な同期方法をいくつか紹介します：
...
多数の代替同期オプションが[コミュニティプラグインディレクトリ](https://community.obsidian.md/search?type=plugin&categories=syncing)で見つかります。
...
- **iCloud Driveを有効にする**:
    - macOSの場合: **システム設定 → Apple ID → iCloud → iCloud Drive** に移動します。
...
- **iCloudに新しい保管庫を作成する**:
    ...
    - iOSの場合:
        ...
        4. **作成**をタップします。
- **保管庫フォルダをダウンロード済みに保つ**（任意ですが、読み込み時間を短縮するために強く推奨されます）:
    - macOS 15（Sequoia）以降の場合: iCloud Driveの**Obsidian**フォルダを右クリックし、**ダウンロードを維持**を選択します。
    - macOS 14（Sonoma）以前の場合: iCloud設定で**Macストレージを最適化**を無効にします。この設定はObsidianだけでなく、デバイス上のすべてのiCloudストレージに影響します。
- **別のAppleデバイスで保管庫を開く**: 
