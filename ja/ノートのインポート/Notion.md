...
cssclasses:
  - soft-embed
---
Obsidianでは、[[インポーター|インポータープラグイン]]を使用してNotionからデータを簡単に移行できます。Notionのワークスペースは、Obsidianやその他多くのアプリでオフラインでも使える耐久性のあるMarkdownファイルに変換されます。

ObsidianではNotionデータのインポートに2つの方法を提供しています：

1. **Notionアカウント（推奨）**はワークスペースに直接接続し、データベースや数式を保持して[[Basesの紹介|ベース]]に変換できます。Notionのインテグレーショントークンとインターネット接続が必要です。
2. **ファイルインポート**はNotionの `.zip` エクスポートファイルを使用します。データベースは保持されませんが、APIトークンやインターネット接続は不要です。

## インポートページのカスタマイズ

どちらのインポート方法でも、インポート開始前にプレビューが表示されます。生成されたテンプレートを編集したり、保管庫からMarkdownテンプレートを選択したりできます。カバーやデータベースのプロパティ名などの設定は、プレビューに表示される生成プロパティを更新します。[[インポーターテンプレート]]を参照してください。

## Notionアカウントからインポート

### Notion APIアクセストークンの作成

API経由でNotionデータにアクセスするには、アクセストークンが必要です。このステップは約2分で完了します。
...
6. **APIトークン**で**リンク…**をクリックして新しいシークレットを追加します。**ID**には `notion` のような名前を付け、**シークレット**にはNotionの**Access token**を貼り付けます。
7. **ロード**をクリックしてインポートしたいデータベースとページを選択します。
8. インポートオプションを確認・編集します。
9. **Import**を選択して、生成されたテンプレートとページからのプレビュー例を確認します。
...
1. Notionサイドバー上部の**Settings**に移動します。
...
[[インポーターテンプレート]]を使用して、Notionデータのインポート方法を完全にカスタマイズできます。
