...
Kantojen funktiot noudattavat JavaScriptin toimintaa. Täydelliset viitedokumentit löytyvät [MDN Web Docs](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference) -sivustolta.

[[Funktiot#Yleiset|Yleisten]] funktioiden lisäksi useimmat funktiot riippuvat muokattavan arvon tyypistä:

- [[Funktiot#Mikä tahansa -tyyppi|Mikä tahansa]]
- [[Funktiot#Päiväys-tyyppi|Päiväys]]
- [[Funktiot#Merkkijono-tyyppi|Merkkijono]]
- [[Funktiot#Numero-tyyppi|Numero]]
- [[Funktiot#Luettelo-tyyppi|Luettelo]]
- [[Funktiot#Linkki-tyyppi|Linkki]]
- [[Funktiot#Tiedosto-tyyppi|Tiedosto]]
- [[Funktiot#Objekti-tyyppi|Objekti]]
- [[Funktiot#Säännöllinen lauseke -tyyppi|Säännöllinen lauseke]]
...
- Erikseen jäsentämistä ei tarvita päiväysaritmetiikassa (esimerkiksi `now() + '1d'`), mutta se on tarpeen suoritettaessa aritmeettisia operaatioita kestoille (esimerkiksi `now() + (duration('1d') * 2)`).
...
- `condition` on arvioitava lauseke.
- `trueResult` on tulos, jos `condition` on tosi.
- `falseResult` on valinnainen tulos, jos `condition` on epätosi. Jos jätetään pois, oletusarvo on `null`.
- Palauttaa `trueResult`-arvon, jos `condition` on tosi tai totuudenmukainen arvo, muuten palauttaa `falseResult`.
...
- Voit halutessasi antaa `display`-parametrin asettaaksesi linkin näytettävän tekstin.
...
- Käytä tätä funktiota, kun määre sisältää holvin eri tiedostoissa sekoituksen merkkijonoja ja luetteloita.
...
- Palauttaa päiväysobjektin nykyhetkestä.
...
- Palauttaa päiväysobjektit millisekunteina Unix-epookista.
- Palauttaa totuusarvot lukuina `1` tai `0`.
- Jäsentää merkkijonot luvuiksi ja palauttaa virheen, jos merkkijono ei ole kelvollinen luku.
...
- Palauttaa päiväysobjektin nykyisestä päivämäärästä. Kellonaikaosuus on asetettu keskiyöhön.
...
- Palauttaa kellonaikaosuuden merkkijonona.
...
- `replacement` on arvo, jolla löydetyt osumat korvataan. Kun `pattern` on säännöllinen lauseke, voit viitata kaappausryhmiin `replacement`-arvossa käyttämällä `$1`, `$2` ja niin edelleen.
...
- Esimerkki kaappausryhmillä: `"John Smith".replace(/(\w+) (\w+)/, "$2, $1")` palauttaa `"Smith, John"`.
...
- Suodattaa luettelon ja säilyttää vain elementit, joille lauseke on tosi.
...
- Muuntaa jokaisen luettelon elementin lausekkeen avulla.
- `value` on luettelon kohteen arvo.
- `index` on nykyisen arvon indeksi.
...
- Tiivistää luettelon yhdeksi arvoksi suorittamalla lausekkeen jokaiselle elementille. Lausekkeen on palautettava seuraava `acc`-arvo. Käytä `value`-muuttujaa nykyiselle elementille, `index`-muuttujaa sen sijainnille ja `acc`-muuttujaa tähänastiselle kertyneelle tulokselle.
...
- Palauttaa `true`, jos `link`-linkin edustamalla tiedostolla on linkki `file`-tiedostoon.

## Tiedosto-tyyppi

Funktioita, joita voit käyttää holvin tiedoston kanssa.
...
- `display` on valinnainen linkin näytettävä teksti.
...
- Palauttaa `true`, jos tiedostolla on annettu määre.
