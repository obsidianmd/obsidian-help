...
Päivitä uusimpaan [[Päivitä Obsidian|Obsidianin asennusohjelman versioon]] (1.11.7) ja uusimpaan [[Ennakkojulkaisuversiot|ennakkojulkaisuversioon]] (1.12.x).
...
- Varmista, että käytät uusinta [[Päivitä Obsidian|Obsidianin asennusohjelman versiota]] (1.12.4 tai uudempi).
- Käynnistä terminaali uudelleen CLI:n rekisteröinnin jälkeen, jotta PATH-muutokset tulevat voimaan.
- Obsidianin on oltava käynnissä. CLI yhdistää käynnissä olevaan Obsidian-instanssiin. Jos Obsidian ei ole käynnissä, ensimmäisen CLI-komennon pitäisi käynnistää sovellus.

### Windows

Obsidian CLI Windowsilla vaatii Obsidian 1.12.4+ -asennusohjelman. Katso [[Päivitä Obsidian|Asennusohjelman version päivitys]].

Windows käyttää terminaalin uudelleenohjaimea, joka yhdistää Obsidianin stdin/stdout-virtaan asianmukaisesti. Tämä on tarpeen, koska Obsidian toimii normaalisti graafisena sovelluksena, joka ei ole yhteensopiva terminaalitulosteiden kanssa Windowsilla. Kun asennat Obsidian 1.12.4+:n, `Obsidian.com`-terminaalin uudelleenohjain lisätään kansioon, johon asensit `Obsidian.exe`-tiedoston.

### macOS

CLI-rekisteröinti lisää Obsidianin binäärihakemiston PATH-muuttujaasi `~/.zprofile`-tiedoston kautta. Jos sinulla on ongelmia, tarkista seuraavat:

`~/.zprofile`-tiedostosi tulisi sisältää seuraava rivi. Jos se puuttuu, voit lisätä sen manuaalisesti:

```
export PATH="$PATH:/Applications/Obsidian.app/Contents/MacOS"
```

#### Vaihtoehtoiset komentotulkit

CLI-rekisteröinti muokkaa vain `~/.zprofile`-tiedostoa, jota käyttää zsh (macOS:n oletuskomentotulkki). Jos käytät eri komentotulkkia, lisää Obsidianin binäärihakemisto komentotulkkisi asetustiedostoon manuaalisesti:

- Bash: lisää `export PATH="$PATH:/Applications/Obsidian.app/Contents/MacOS"` tiedostoon `~/.bash_profile`
- Fish: suorita `fish_add_path /Applications/Obsidian.app/Contents/MacOS`


### Linux

CLI-rekisteröinti luo symbolisen linkin kohteeseen `/usr/local/bin/obsidian`, joka osoittaa Obsidianin binääriin (vaatii sudon).

#### AppImage

AppImage-asennuksissa symbolinen linkki osoittaa `.AppImage`-tiedostoon sisäisen binäärin sijaan, koska liitospolku muuttuu joka käynnistyksellä. Jos sudo epäonnistuu, symbolinen linkki luodaan varavaihtoehtona kohteeseen `~/.local/bin/obsidian`. Jos sinulla on ongelmia, tarkista seuraavat.
...
```
sudo ln -s /polku/obsidianiin /usr/local/bin/obsidian
```

Jos symbolinen linkki luotiin `~/.local/bin/`-hakemistoon, varmista että hakemisto on PATH-muuttujassasi. Lisää seuraava `~/.bashrc`- tai `~/.zshrc`-tiedostoosi:

```
export PATH="$PATH:$HOME/.local/bin"
```

Jos symbolinen linkki rikkoutuu `.AppImage`-tiedoston siirtämisen tai uudelleennimeämisen jälkeen, rekisteröi CLI uudelleen tai päivitä symbolinen linkki manuaalisesti.

#### Snap

Snap-paketti tallentaa insider-kokeiluversioiden tiedot omaan käyttäjätietohakemistoonsa. Jos CLI ei havaitse insider-`.asar`-tiedostoa, aseta `XDG_CONFIG_HOME` osoittamaan Snapin asetuspolkuun:

```
export XDG_CONFIG_HOME="$HOME/snap/obsidian/current/.config"
```

Lisää tämä `~/.bashrc`- tai `~/.zshrc`-tiedostoosi tehdäksesi siitä pysyvän.


#### Flatpak

Obsidian yrittää tehdä tämän automaattisesti, mutta alla ovat manuaaliset ohjeet. Jos kyseessä on järjestelmätason asennus:

```
ln -s /var/lib/flatpak/exports/bin/md.obsidian.Obsidian ~/.local/bin/obsidian
```

Jos kyseessä on käyttäjätason asennus:
