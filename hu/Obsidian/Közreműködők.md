---
permalink: credits
---
A nevek nem a hozzájárulás mértéke, hanem ábécé sorrend szerint vannak felsorolva (legalábbis megpróbáljuk!).

## Alapcsapat

Az alapcsapat, amely az Obsidian-t építi és karbantartja.

- codedmart (Brandon Martin)
- dharam (Dharam Kapila)
- kepano ([Steph Ango](https://stephanango.com/))
- Licat (Shida Li)
- liam ([Liam Cain](https://liamca.in/)) <span class='flair'>Maker of Calendar</span> <span class='flair'>Plugin API Master</span> <span class='flair'>Volcano Veteran</span>
- Silver ([Erica Xu](https://ericaxu.com))<span class='flair'>Team Little Silvers</span>
- Sigrunixia ([Rebbecca Bishop](https://tenebrousdragon.com/home))<span class='flair'>Sorcerous Support Dragon</span> <span class='flair'>TTRPG Enthusiast</span>
- tgrosinger ([Tony Grosinger](https://grosinger.net))

## Közreműködők

Részmunkaidős fejlesztők, akik segítenek az Obsidian építésében.

- cmptr (Aaron Bell)
- mgmeyers ([Matthew Meyers](https://matthewmeye.rs/))

## Közösségi felülvizsgálat

A [közösségi](https://community.obsidian.md/) bővítmény/téma felülvizsgálati folyamat és eszközök közreműködői.

- Fevol
- mnaoumov ([Michael Naumov](https://mnaoumov.dev/))
- Saberzero1 (Emile Bangma)
- Zachatoo ([Zach Young](https://zachyoung.dev/)) <span class='flair'>Templater Templar</span><span class='flair'>Board Gamer</span>

## Moderálás

A moderátorok az Obsidian Fórumot, a Discordot és a Redditet kezelik.

- argentum<span class='flair'>Team Little Silvers</span> <span class='flair'>Linux master</span> <span class='flair'>Zotero master</span> ^a4b3a2
- CawlinTeffid ([Colin Tedford](https://colintedford.com/)) <span class='flair'>embedded-search ambassador</span>
- cotemaxime ([Maxime Côté](https://www.maximecote.me/))<span class='flair'>Linux master</span>
- koala<span class='flair'>Extreme Bug Finder</span>
- Leah ([Leah Ferguson](http://leahferguson.com))<span class='flair'>Roll for initiative!</span> <span class='flair'>Non-techy Apple geek</span>
- rigmarole ([Chris Lesage](http://rigmarolestudio.com))<span class='flair'>Forum master</span>
- Roam ([Roman Soriano](https://leftovers.blog/)) <span class='flair'>Not <i>that</i> one</span>
- Trainbuffer <span class='flair'> ferroequinologist</span> <span class='flair'>Travels by rail</span>
- WhiteNoise<span class='flair'>Think Tank</span> <span class='flair'>Zotero master</span> <span class='flair'>Bug report exterminator</span>
- Carl the Turtle<span class='flair'>Online 24/7</span> <span class='flair'>Easily triggered</span> <span class='flair'>Good and pure</span> 

## Alumni

Korábbi moderátorok és csapattagok, akik új kalandokba vágtak!

- Eleanor ([Eleanor Konik](https://eleanorkonik.com))<span class='flair'>Halp Obsidian turned me into a dev</span>
- joethei ([Johannes Theiner](https://joethei.xyz/)) <span class='flair'>Plugin API Master</span>
- mediapathic ([Steen Comer](http://mediapathic.net/))<span class='flair'>Bad Cop</span>
- ryanjamurphy ([Ryan](https://fulcra.design/) [J. A. Murphy](https://axle.design/))<span class='flair'>That funny guy</span> <span class='flair'>Apple Fanboy</span> <span class='flair'>Forum master</span>

## Bővítmény-inspirációk

Köszönet ezeknek a bővítményfejlesztőknek az úttörő funkciókért, amelyeket az Obsidian adaptált:

- pjeby a Hotkey Helper bővítményért
- javalent az Admonition bővítményért
- javalent a Settings Search bővítményért
- NothingIsLost a CodeMirror 6 fejlesztésekért
- mgmeyers az Indentation Guides bővítményért
- blacksmithgu a Dataview-ért

## Build-ek

- jstone a Flathub build-ek karbantartásáért
- adamgibbins és ran-dall a homebrew build-ek karbantartásáért

## Fordítók

Lásd még a [fordítói közreműködők](https://github.com/obsidianmd/obsidian-translations/graphs/contributors?from=7%2F31%2F2020) teljes listáját.

- Andrea Brandi (olasz)
- Anne-Katrin aka Mephi (német)
- bongudth (vietnámi)
- canzi-teacher, 蚕子 (egyszerűsített kínai)
- cor-bee, Ілля Пишняк (ukrán)
- cvyl, Mikka (holland)
- Daniel Mathiot (francia)
- Diego Tres (brazil portugál)
- Haitham Mohamed (arab)
- Henrik Falk (dán)
- Joffrey Schroeder (francia) 
- JxhnnyUt8h (orosz)
- k-andzhanovskii, Константин Анджановский (orosz)
- lisachev, Сергей Лисачев (orosz)
- mafsi, Patrick Danilevici (román)
- Manuel Arias Almada (spanyol)
- Moyf, 墨一 (egyszerűsített kínai)
- Muhammad Mauli Mubassari (indonéz)
- Olivier Anthonioz (francia) 
- Ryooo，睿悠ryooo (egyszerűsített kínai)
- Sakvipubp Suy (khmer)
- smilorent, Silviu Lorenț (román)
- Solyta Teng (khmer)
- taksas, 中山 拓海 (japán)
- VKondakoff, Валерий Кондаков (orosz)
- yo-goto, ぱだわん (japán)
- Zachary Shirmohammadli (perzsa) 

## Támogató szervezetek

Az Obsidian-t [[Kereskedelmi licenc|kereskedelmi licenccel]] támogató szervezetek az [Enterprise](https://obsidian.md/enterprise/) oldalunkon szerepelnek. 

## Dokumentáció

A súgó széf első verzióját Silver írta. A későbbi részét [@mediapathic](http://mediapathic.net) írta, Reggie nagy segítségével. A Discord és fórum közösség számtalan tagjának figyelmes szemei is javításokat küldtek.

A dokumentációt egy ideig [Marcus Olsson](https://marcus.se.net/) tartotta karban. Jelenleg az Obsidian csapat tartja karban.

Pull requesteket fogadunk a súgó széfhez a [dokumentáció repónkban](https://github.com/obsidianmd/obsidian-docs/). Az issue-k is szívesen fogadottak.

### Add hozzá a neved

Ha fordító vagy, kérjük küldj egy pull requestet, hogy hozzáadd a „Neved (hozzájárulás nyelve)" formátumot a fenti részhez. Kérjük, tartsd a neveket ábécé sorrendben, köszönjük!

Ha a neved nem angolul van, és nem sorolható be ábécé sorrendbe, használd ezt a formátumot: `github-felhasználóneved, [neved a saját nyelveden] (Nyelv neve)`.

## Harmadik féltől származó elismerések

Az Obsidian az alábbi nyílt forráskódú könyvtárakat használja (ábécé sorrendben):

### Capacitor

Verzió: `5.x`
MIT License
Copyright (c) 2017-present Drifty Co.

### CodeMirror

Vim verzió: `6.0.0`
MIT License
Copyright (C) 2018 by Marijn Haverbeke <marijnh@gmail.com>, Adrian Heine <mail@adrianheine.de>, and others

### DOMPurify

https://github.com/cure53/DOMPurify
Licensed under the [Mozilla Public License version 2.0](http://mozilla.org/MPL/2.0/)

### Electron

Verzió: `37.3.0`
MIT License
Copyright (c) Electron contributors
Copyright (c) 2013-2020 GitHub Inc.

### i18next

MIT License
Copyright (c) 2022 i18next

### Lezer

MIT License
Copyright (C) 2018-2021 by Marijn Haverbeke <marijnh@gmail.com> and others

### Lucide

Verzió: `0.446.0`
ISC License
Copyright (c) 2020, Lucide Contributors ^lucide

### MathJax

Apache License 2.0

### Mermaid

Verzió: `11.4.1`
MIT License
Copyright (c) 2014 - 2022 Knut Sveidqvist

### Moment.js

Verzió: `2.29.4`
MIT License
Copyright (c) JS Foundation and other contributors

### pdf.js

Apache License 2.0

### PixiJS

MIT License
Copyright (c) 2013-2017 Mathew Groves, Chad Engler

### Prism

Verzió: `1.29.0`
MIT License
Copyright (c) 2012 Lea Verou

### rbush

MIT License
Copyright (c) 2016 Vladimir Agafonkin

### remark

MIT License
Copyright (c) 2014-2020 Titus Wormer <tituswormer@gmail.com>
Copyright (c) 2011-2014, Christopher Jeffrey (https://github.com/chjj/)

### reveal.js

Verzió: `4.3.1`
The MIT License
Copyright (C) 2011-2022 Hakim El Hattab, http://hakim.se, and reveal.js contributors

### scrypt

Apache License 2.0

### Turndown

MIT License
Copyright (c) 2017 Dom Christie

### Webpack

MIT License
Copyright JS Foundation and other contributors

### YAML

Verzió: `2.7.0`
ISC License
Copyright Eemeli Aro <eemeli@gmail.com>

## Korábbi logó

Az Obsidian v1.3.5 előtt használt logója az alábbi alapján készült:

[Crystal shard icon](https://icon-icons.com/icon/Crystal-Shard/88819)
Készítő: [Chanut is Industries](https://icon-icons.com/users/W52nHhY3W1VlvwyJTwS4d/icon-sets/ "Designer")
Licenc: Attribution 3.0 Unported (CC BY 3.0)
