---
permalink: credits
---
Los nombres no están listados por cantidad de contribución, sino alfabéticamente (¡o al menos lo intentamos!).

## Equipo

### Desarrollo

- Licat (Shida Li)<span class='flair mod-pop'>Obsidian office cat</span>
- liam ([Liam Cain](https://liamca.in/)) <span class='flair mod-pop'>Maker of Calendar</span> <span class='flair mod-pop'>Plugin API Master</span> <span class='flair mod-pop'>Volcano Veteran</span>
- joethei ([Johannes Theiner](https://joethei.xyz/)) <span class='flair mod-pop'>Plugin API Master</span>
- tgrosinger ([Tony Grosinger](https://grosinger.net))
- mgmeyers ([Matthew Meyers](https://matthewmeye.rs/))

### Producto y operaciones

- Silver ([Erica Xu](https://ericaxu.com))<span class='flair mod-pop'>Team Little Silvers</span>
- kepano ([Steph Ango](https://stephanango.com/))
- Sigrunixia ([Rebbecca Bishop](https://tenebrousdragon.com/home))<span class='flair mod-pop'>Sorcerous Support Dragon</span> <span class='flair mod-pop'>TTRPG Enthusiast</span>

## Moderadores

- argentum<span class='flair mod-pop'>Team Little Silvers</span> <span class='flair mod-pop'>Linux master</span> <span class='flair mod-pop'>Zotero master</span> ^a4b3a2
- cotemaxime ([Maxime Côté](https://www.maximecote.me/))<span class='flair mod-pop'>Linux master</span>
- koala<span class='flair mod-pop'>Extreme Bug Finder</span>
- Leah ([Leah Ferguson](http://leahferguson.com))<span class='flair mod-pop'>Roll for initiative!</span> <span class='flair mod-pop'>Non-techy Apple geek</span>
- rigmarole ([Chris Lesage](http://rigmarolestudio.com))<span class='flair mod-pop'>Forum master</span>
- ryanjamurphy ([Ryan](https://fulcra.design/) [J. A. Murphy](https://axle.design/))<span class='flair mod-pop'>That funny guy</span> <span class='flair mod-pop'>Apple Fanboy</span> <span class='flair mod-pop'>Forum master</span>
- WhiteNoise<span class='flair mod-pop'>Think Tank</span> <span class='flair mod-pop'>Zotero master</span> <span class='flair mod-pop'>Bug report exterminator</span>
- CawlinTeffid ([Colin Tedford](https://colintedford.com/)) <span class='flair mod-pop'>embedded-search ambassador</span>
- Trainbuffer <span class='flair mod-pop'> ferroequinologist</span> <span class='flair mod-pop'>Travels by rail</span>
- Roam ([Roman Soriano](https://leftovers.blog/)) <span class='flair mod-pop'>Not <i>that</i> one</span>
- Zachatoo ([Zach Young](https://zachyoung.dev/)) <span class='flair mod-pop'>Templater Templar</span><span class='flair mod-pop'>Board Gamer</span>
- Carl the Turtle<span class='flair mod-pop'>Online 24/7</span> <span class='flair mod-pop'>Easily triggered</span> <span class='flair mod-pop'>Good and pure</span> <span class='flair mod-pop' style='background-color:#0a8c28'>Go green or go home</span>

### Retirados

- Eleanor ([Eleanor Konik](https://eleanorkonik.com))<span class='flair mod-pop'>Halp Obsidian turned me into a dev</span>
- mediapathic ([Steen Comer](http://mediapathic.net/))<span class='flair mod-pop'>Bad Cop</span>

## Inspiraciones de complementos

Créditos a estos desarrolladores de complementos por ser pioneros en funcionalidades que fueron adaptadas a Obsidian:

- pjeby por el complemento Hotkey Helper
- javalent por el complemento Admonition
- NothingIsLost por las mejoras de CodeMirror 6
- mgmeyers por el complemento Indentation Guides
- blacksmithgu por Dataview

## Compilaciones

- jstone por mantener nuestras compilaciones en flathub
- adamgibbins y ran-dall por mantener nuestras compilaciones en homebrew

## Traductores

- Andrea Brandi (italiano)
- Anne-Katrin aka Mephi (alemán)
- bongudth (vietnamita)
- canzi-teacher, 蚕子 (chino simplificado)
- cor-bee, Ілля Пишняк (ucraniano)
- cvyl, Mikka (neerlandés)
- Daniel Mathiot (francés)
- Diego Tres (portugués brasileño)
- Haitham Mohamed (árabe)
- Henrik Falk (danés)
- Joffrey Schroeder (francés)
- JxhnnyUt8h (ruso)
- k-andzhanovskii, Константин Анджановский (ruso)
- lisachev, Сергей Лисачев (ruso)
- mafsi, Patrick Danilevici (rumano)
- Manuel Arias Almada (español)
- Moyf, 墨一 (Chinese Simplified)
- Muhammad Mauli Mubassari (indonesio)
- Olivier Anthonioz (francés)
- Ryooo，睿悠ryooo (chino simplificado)
- Sakvipubp Suy (jemer)
- smilorent, Silviu Lorenț (rumano)
- Solyta Teng (jemer)
- taksas, 中山 拓海 (japonés)
- VKondakoff, Валерий Кондаков (ruso)
- yo-goto, ぱだわん (japonés)
- Zachary Shirmohammadli (persa)

## Documentación

La primera versión de la bóveda de ayuda fue escrita por Silver. Posteriormente, parte de ella fue escrita por [@mediapathic](http://mediapathic.net), con una gran ayuda de Reggie. También recibimos correcciones de los muchos ojos atentos en la comunidad de Discord y el foro.

La documentación fue mantenida por [Marcus Olsson](https://marcus.se.net/) durante un tiempo. Actualmente es mantenida por el equipo de Obsidian.

Ahora aceptamos pull requests a la bóveda de ayuda en [nuestro repositorio de documentación](https://github.com/obsidianmd/obsidian-docs/). Los issues son bienvenidos.

### Añade tu nombre

Si eres traductor, por favor haz un pull request para añadir "Tu Nombre (idioma contribuido)" a la sección anterior. Por favor mantén la lista de nombres en orden alfabético, ¡gracias!

Si tu nombre no está en inglés y no puede ser listado alfabéticamente, usa este formato: `tu-nombre-de-usuario-en-github, [tu nombre en tu idioma] (Nombre del idioma)`.

## Reconocimientos a terceros

Obsidian utiliza las bibliotecas de código abierto que se indican a continuación (en orden alfabético):

### Capacitor

Versión `5.x`
Licencia MIT
Copyright (c) 2017-present Drifty Co.

### CodeMirror

Vim Versión `6.0.0`
Licencia MIT
Copyright (C) 2018 by Marijn Haverbeke <marijnh@gmail.com>, Adrian Heine <mail@adrianheine.de>, and others

### DOMPurify

https://github.com/cure53/DOMPurify
Licenciado bajo la [Licencia Pública de Mozilla versión 2.0](http://mozilla.org/MPL/2.0/)

### Electron

Versión `37.3.0`
Licencia MIT
Copyright (c) Electron contributors
Copyright (c) 2013-2020 GitHub Inc.

### i18next

Licencia MIT
Copyright (c) 2022 i18next

### Lezer

Licencia MIT
Copyright (C) 2018-2021 by Marijn Haverbeke <marijnh@gmail.com> and others

### Lucide

Versión `0.446.0`
Licencia ISC
Copyright (c) 2020, Lucide Contributors ^lucide

### MathJax

Licencia Apache 2.0

### Mermaid

Versión `11.4.1`
Licencia MIT
Copyright (c) 2014 - 2022 Knut Sveidqvist

### Moment.js

Versión `2.29.4`
Licencia MIT
Copyright (c) JS Foundation and other contributors

### pdf.js

Licencia Apache 2.0

### PixiJS

Licencia MIT
Copyright (c) 2013-2017 Mathew Groves, Chad Engler

### Prism

Versión `1.29.0`
Licencia MIT
Copyright (c) 2012 Lea Verou

### rbush

Licencia MIT
Copyright (c) 2016 Vladimir Agafonkin

### remark

Licencia MIT
Copyright (c) 2014-2020 Titus Wormer <tituswormer@gmail.com>
Copyright (c) 2011-2014, Christopher Jeffrey (https://github.com/chjj/)

### reveal.js

Versión `4.3.1`
Licencia MIT
Copyright (C) 2011-2022 Hakim El Hattab, http://hakim.se, and reveal.js contributors

### scrypt

Licencia Apache 2.0

### Turndown

Licencia MIT
Copyright (c) 2017 Dom Christie

### Webpack

Licencia MIT
Copyright JS Foundation and other contributors

### YAML

Versión `2.7.0`
Licencia ISC
Copyright Eemeli Aro <eemeli@gmail.com>

## Logotipo anterior

El logotipo de Obsidian utilizado antes de la v1.3.5 fue adaptado de:

[Crystal shard icon](https://icon-icons.com/icon/Crystal-Shard/88819)
Por [Chanut is Industries](https://icon-icons.com/users/W52nHhY3W1VlvwyJTwS4d/icon-sets/ "Designer")
Licencia: Attribution 3.0 Unported (CC BY 3.0)
