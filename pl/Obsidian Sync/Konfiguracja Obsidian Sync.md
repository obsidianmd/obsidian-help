...
W tej sekcji utworzysz nowy [[Sejfy lokalne i zdalne|zdalny sejf]] i połączysz go z istniejącym lokalnym sejfem. Nie musisz tworzyć nowego, pustego lokalnego sejfu, aby korzystać z Obsidian Sync w tym celu.

> [!info] Czy Twój obecny sejf znajduje się w folderze iCloud, OneDrive, Dropbox lub innej usługi synchronizacji? Jeśli **tak** lub **nie masz pewności**, przeczytaj [[Najczęściej zadawane pytania#Czy mogę używać zewnętrznej synchronizacji z Obsidian Sync?|ten artykuł]] oraz [[Przejście na Obsidian Sync]] przed kontynuowaniem.
...
1. Otwórz **[[Ustawienia]]**.
2. Na panelu bocznym w sekcji **Opcje** wybierz **Wbudowane wtyczki**.
...
> Gdy Obsidian Sync zakończy synchronizację, w prawym dolnym rogu (komputer) lub na prawym panelu bocznym (urządzenie mobilne) pojawi się zielone kółko ze znacznikiem wyboru ![[obsidian-icon-sync-synced.svg#icon]]. Dziennik synchronizacji wyświetli również komunikat „W pełni zsynchronizowano" jako jeden z najnowszych wpisów.
>
...
### Synchronizacja sejfu z przełącznika sejfów

Jeśli właśnie zainstalowałeś Obsidian, po otwarciu programu zobaczysz [[Zarządzaj sejfami|Przełącznik sejfów]]. Aby utworzyć nowy lokalny sejf z zawartości zdalnego sejfu, wykonaj następujące kroki.
...
5. Zostaniesz poproszony o wybranie nazwy dla lokalnego sejfu, który zostanie utworzony na urządzeniu do przechowywania tych danych. Wprowadź wybraną nazwę.
	1. Jeśli używasz [[Obsidian URI]], warto użyć tej samej nazwy co lokalny sejf na innym urządzeniu.
6. Wybierz **Stwórz**.
...
Jeśli już utworzyłeś lokalny sejf na tym urządzeniu i chcesz połączyć go ze zdalnym sejfem, instrukcje są bardzo podobne do [[#Konfiguracja Obsidian Sync po raz pierwszy]].
...
6. Zostaniesz poproszony o rozpoczęcie synchronizacji. Zaleca się poczekać i najpierw dostosować ustawienia synchronizacji. Jeśli chcesz zsynchronizować cały sejf na urządzenie w obecnym stanie, możesz wybrać **Rozpocznij synchronizację**.

> [!warning] Jeśli sejf na Twoim urządzeniu zawiera już jakieś notatki (niezalecane), zostaniesz ostrzeżony, że notatki te zostaną scalone przed kontynuowaniem. Konflikty zostaną rozwiązane za pomocą [[Rozwiązywanie problemów z Obsidian Sync#Rozwiązywanie konfliktów|mechanizmu rozwiązywania konfliktów Sync]].
...
2. Wybierz **Sync** na panelu bocznym.
