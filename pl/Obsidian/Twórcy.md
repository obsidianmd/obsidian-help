---
permalink: credits
---
Nazwiska nie są wymienione według wielkości wkładu, lecz alfabetycznie (a przynajmniej staramy się to utrzymać!).

## Zespół główny

Główny zespół, który tworzy i utrzymuje Obsidian.

- codedmart (Brandon Martin)
- dharam (Dharam Kapila)
- kepano ([Steph Ango](https://stephanango.com/))
- Licat (Shida Li)
- liam ([Liam Cain](https://liamca.in/)) <span class='flair'>Maker of Calendar</span> <span class='flair'>Plugin API Master</span> <span class='flair'>Volcano Veteran</span>
- Silver ([Erica Xu](https://ericaxu.com))<span class='flair'>Team Little Silvers</span>
- Sigrunixia ([Rebbecca Bishop](https://tenebrousdragon.com/home))<span class='flair'>Sorcerous Support Dragon</span> <span class='flair'>TTRPG Enthusiast</span>
- tgrosinger ([Tony Grosinger](https://grosinger.net))

## Współtwórcy

Deweloperzy pracujący w niepełnym wymiarze, którzy pomagają tworzyć Obsidian.

- cmptr (Aaron Bell)
- mgmeyers ([Matthew Meyers](https://matthewmeye.rs/))

## Recenzja społeczności

Współtwórcy procesu recenzji wtyczek/motywów [społeczności](https://community.obsidian.md/) oraz narzędzi.

- Fevol <span class='flair'>Plugin API Journeyman</span><span class='flair'>Canonically Commentated</span>
- mnaoumov ([Michael Naumov](https://mnaoumov.dev/))
- saberzero1 ([Emile Bangma](https://emilebangma.com)) <span class='flair'>Quartz Quartermaster</span> <span class='flair'>ESLint Legislator</span>
- Zachatoo ([Zach Young](https://zachyoung.dev/)) <span class='flair'>Templater Templar</span><span class='flair'>Board Gamer</span>

## Moderacja

Moderatorzy zarządzają forum Obsidian, Discordem i Redditem.

- argentum<span class='flair'>Team Little Silvers</span> <span class='flair'>Linux master</span> <span class='flair'>Zotero master</span> ^a4b3a2
- CawlinTeffid ([Colin Tedford](https://colintedford.com/)) <span class='flair'>embedded-search ambassador</span>
- cotemaxime ([Maxime Côté](https://www.maximecote.me/))<span class='flair'>Linux master</span>
- koala<span class='flair'>Extreme Bug Finder</span>
- Leah ([Leah Ferguson](http://leahferguson.com))<span class='flair'>Roll for initiative!</span> <span class='flair'>Non-techy Apple geek</span>
- rigmarole ([Chris Lesage](http://rigmarolestudio.com))<span class='flair'>Forum master</span>
- Roam ([Roman Soriano](https://leftovers.blog/)) <span class='flair'>Not <i>that</i> one</span>
- Trainbuffer <span class='flair'> ferroequinologist</span> <span class='flair'>Travels by rail</span>
- WhiteNoise<span class='flair'>Think Tank</span> <span class='flair'>Zotero master</span> <span class='flair'>Bug report exterminator</span>
- Carl the Turtle<span class='flair'>Online 24/7</span> <span class='flair'>Easily triggered</span> <span class='flair'>Good and pure</span> 

## Alumni

Byli moderatorzy i członkowie zespołu, którzy ruszyli na nowe przygody!

- Eleanor ([Eleanor Konik](https://eleanorkonik.com))<span class='flair'>Halp Obsidian turned me into a dev</span>
- joethei ([Johannes Theiner](https://joethei.xyz/)) <span class='flair'>Plugin API Master</span>
- mediapathic ([Steen Comer](http://mediapathic.net/))<span class='flair'>Bad Cop</span>
- ryanjamurphy ([Ryan](https://fulcra.design/) [J. A. Murphy](https://axle.design/))<span class='flair'>That funny guy</span> <span class='flair'>Apple Fanboy</span> <span class='flair'>Forum master</span>

## Inspiracje wtyczkami

Podziękowania dla poniższych twórców wtyczek, którzy wprowadzili funkcjonalności zaadaptowane później w Obsidian:

- pjeby za wtyczkę Hotkey Helper
- javalent za wtyczkę Admonition
- javalent za wtyczkę Settings Search
- NothingIsLost za ulepszenia CodeMirror 6
- mgmeyers za wtyczkę Indentation Guides
- blacksmithgu za Dataview

## Kompilacje

- jstone za utrzymywanie naszych kompilacji na Flathub
- adamgibbins i ran-dall za utrzymywanie naszych kompilacji na homebrew

## Tłumacze

Zobacz również pełną listę [współtwórców tłumaczeń](https://github.com/obsidianmd/obsidian-translations/graphs/contributors?from=7%2F31%2F2020).

- Andrea Brandi (włoski)
- Anne-Katrin aka Mephi (niemiecki)
- bongudth (wietnamski)
- canzi-teacher, 蚕子 (chiński uproszczony)
- cor-bee, Ілля Пишняк (ukraiński)
- cvyl, Mikka (niderlandzki)
- Daniel Mathiot (francuski)
- Diego Tres (portugalski brazylijski)
- Haitham Mohamed (arabski)
- Henrik Falk (duński)
- Joffrey Schroeder (francuski) 
- JxhnnyUt8h (rosyjski)
- k-andzhanovskii, Константин Анджановский (rosyjski)
- lisachev, Сергей Лисачев (rosyjski)
- mafsi, Patrick Danilevici (rumuński)
- Manuel Arias Almada (hiszpański)
- Moyf, 墨一 (chiński uproszczony)
- Muhammad Mauli Mubassari (indonezyjski)
- Olivier Anthonioz (francuski) 
- Ryooo，睿悠ryooo (chiński uproszczony)
- Sakvipubp Suy (khmerski)
- smilorent, Silviu Lorenț (rumuński)
- Solyta Teng (khmerski)
- taksas, 中山 拓海 (japoński)
- VKondakoff, Валерий Кондаков (rosyjski)
- yo-goto, ぱだわん (japoński)
- Zachary Shirmohammadli (perski) 

## Wspierające organizacje

Organizacje wspierające Obsidian dzięki [[Licencja komercyjna|licencji komercyjnej]] są prezentowane na naszej stronie [Enterprise](https://obsidian.md/enterprise/). 

## Dokumentacja

Pierwsza wersja skarbca pomocy została napisana przez Silver. Późniejsza część została napisana przez [@mediapathic](http://mediapathic.net), z dużą pomocą Reggiego. Otrzymaliśmy również poprawki od wielu par oczu ze społeczności Discord i forum.

Dokumentacja była przez pewien czas utrzymywana przez [Marcusa Olssona](https://marcus.se.net/). Obecnie jest utrzymywana przez zespół Obsidian.

Przyjmujemy teraz pull requesty do skarbca pomocy w [naszym repozytorium dokumentacji](https://github.com/obsidianmd/obsidian-docs/). Zgłoszenia problemów są mile widziane.

### Dodaj swoje imię

Jeśli jesteś tłumaczem, wyślij pull request, aby dodać „Twoje imię (język, w którym tłumaczysz)" do powyższej sekcji. Prosimy o utrzymanie listy w porządku alfabetycznym, dziękujemy!

Jeśli Twoje imię nie jest w języku angielskim i nie można go łatwo posortować alfabetycznie, użyj tego formatu: `twoja-nazwa-użytkownika-github, [twoje imię w twoim języku] (nazwa języka)`.

## Podziękowania dla stron trzecich

Obsidian korzysta z poniższych bibliotek open source (w kolejności alfabetycznej):

### Capacitor

Wersja `5.x`
Licencja MIT
Copyright (c) 2017-present Drifty Co.

### CodeMirror

Vim Wersja `6.0.0`
Licencja MIT
Copyright (C) 2018 by Marijn Haverbeke <marijnh@gmail.com>, Adrian Heine <mail@adrianheine.de>, and others

### DOMPurify

https://github.com/cure53/DOMPurify
Licencjonowane na podstawie [Mozilla Public License wersja 2.0](http://mozilla.org/MPL/2.0/)

### Electron

Wersja `37.3.0`
Licencja MIT
Copyright (c) Electron contributors
Copyright (c) 2013-2020 GitHub Inc.

### i18next

Licencja MIT
Copyright (c) 2022 i18next

### Lezer

Licencja MIT
Copyright (C) 2018-2021 by Marijn Haverbeke <marijnh@gmail.com> and others

### Lucide

Wersja `0.446.0`
Licencja ISC
Copyright (c) 2020, Lucide Contributors ^lucide

### MathJax

Apache License 2.0

### Mermaid

Wersja `11.4.1`
Licencja MIT
Copyright (c) 2014 - 2022 Knut Sveidqvist

### Moment.js

Wersja `2.29.4`
Licencja MIT
Copyright (c) JS Foundation and other contributors

### pdf.js

Apache License 2.0

### PixiJS

Licencja MIT
Copyright (c) 2013-2017 Mathew Groves, Chad Engler

### Prism

Wersja `1.29.0`
Licencja MIT
Copyright (c) 2012 Lea Verou

### rbush

Licencja MIT
Copyright (c) 2016 Vladimir Agafonkin

### remark

Licencja MIT
Copyright (c) 2014-2020 Titus Wormer <tituswormer@gmail.com>
Copyright (c) 2011-2014, Christopher Jeffrey (https://github.com/chjj/)

### reveal.js

Wersja `4.3.1`
Licencja MIT
Copyright (C) 2011-2022 Hakim El Hattab, http://hakim.se, and reveal.js contributors

### scrypt

Apache License 2.0

### Turndown

Licencja MIT
Copyright (c) 2017 Dom Christie

### Webpack

Licencja MIT
Copyright JS Foundation and other contributors

### YAML

Wersja `2.7.0`
Licencja ISC
Copyright Eemeli Aro <eemeli@gmail.com>

## Poprzednie logo

Logo Obsidian używane przed wersją v1.3.5 zostało zaadaptowane z:

[Crystal shard icon](https://icon-icons.com/icon/Crystal-Shard/88819)
Autorstwa [Chanut is Industries](https://icon-icons.com/users/W52nHhY3W1VlvwyJTwS4d/icon-sets/ "Designer")
Licencja: Attribution 3.0 Unported (CC BY 3.0)
