...
To make the link display different text than its real note name in Preview, use the vertical pipe (`Shift+|`) For example: [[Link wewnętrzny|Custom Link Name in Preview!]] This can be combined with linking to headers, as in [[Zawijanie#By way of example|Example of Folding]].
...
- To see a preview of the linked content without leaving the current note, enable the [[Podgląd stron]] plugin and a popup window will be displayed.
