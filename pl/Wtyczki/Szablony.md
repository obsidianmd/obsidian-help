...
Currently, dates like `{{date:YYYY-MM-DD}}` and `{{time:HH:mm}}` also work in the template file of the [[Codzienne notatki]] and [[Automatyczne tworzenie prefiksów Zettelkasten]] plugins. However, `{{date}}` and `{{time}}` do not work yet.
