...
- `"Line 1\\nLine 2"|unescape` 返回分为两行的文本。
- `'He said \\"hello\\"'|unescape` 返回 `He said "hello"`。
...
使用指定的小数位数以及可选的小数点分隔符和千位分隔符格式化数字。

- `1234.567|number_format:2` 返回 `"1,234.57"`。
- `1234.567|number_format:(2,",",".")` 返回 `"1.234,57"`。
- 数组和对象会被递归格式化。
...
将 HTML 转换为结构化 JSON，包含每个元素的标签、属性和子元素。文本节点使用格式 `{"type":"text","content":"..."}`。可以与 `map`、`object` 和 `template` 等筛选器结合使用，以重塑提取的 HTML。
