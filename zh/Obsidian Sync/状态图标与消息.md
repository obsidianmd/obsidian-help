...
文件名使用了接收操作系统无法存储的[特殊字符或命名约定](https://stackoverflow.com/questions/1976007/what-characters-are-forbidden-in-windows-and-linux-directory-names)。请在源设备上重命名文件，然后让同步服务重新上传重命名后的文件。

对于跨不同操作系统同步的仓库，请避免在文件和文件夹名称中使用以下内容：

- 文件系统通常保留的字符，如 `/`、`\`、`:`、`*`、`?`、`"`、`<`、`>` 和 `|`
- 名称末尾的空格或句点
- Windows 保留的设备名称，如 `CON`、`PRN`、`AUX`、`NUL`、`COM1` 到 `COM9`，或 `LPT1` 到 `LPT9`
- Obsidian 用于链接的字符，如 `#`、`^`、`[` 和 `]`（如果你希望笔记名称对链接友好）
- 文件名中的多个句点 `.` 或表情符号，某些 Android 设备可能会拒绝这些内容

如果不确定，请使用字母、数字、名称中间的普通空格、连字符 `-`、下划线 `_`，以及文件扩展名前的一个句点。
