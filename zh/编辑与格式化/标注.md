...
要创建标注，将 `[!info]` 添加到引用块的第一行即可。其中 `info` 是 _类型标识符_。类型标识符决定了标注的外观。要查看所有可用类型，请参阅[[#支持的类型]]。在[[发布服务简介|Obsidian 发布服务]]中也原生支持标注。
...
你可以使用`插入标注`[[命令面板|命令]]来插入一个默认的 `[!note]` 标注。光标会自动定位到标注名称字段，让你删除默认名称并输入新名称，然后再编辑内容。

要将已有内容包裹在标注中，选择文本（包括列表、代码块等）并运行`插入标注`命令。所选内容将自动包含在标注中。

在[[编辑与预览笔记#实时预览|实时预览]]模式下，你还可以右键点击标注名称来更改标注类型。
...
- `--callout-color` 定义背景颜色。任何有效的 CSS 颜色值均可使用，例如十六进制代码（`#000000`）或 `rgb()` 值。
...
> [!tip]- 更多自定义选项
> 你不仅可以使用 `--callout-color` 和 `--callout-icon`。标注支持标准 CSS 选择器和属性，还有额外的 CSS 变量用于控制边框和标题样式等。
> 
> 例如，隐藏标注的标题：
> 
> ```css
...
> ```
> 
> 查看完整的[标注 CSS 变量列表](https://docs.obsidian.md/Reference/CSS+variables/Editor/Callout)以了解更多选项。

### 支持的类型

Obsidian 内置了多种标注类型。每种类型都带有不同的背景颜色和图标。

要使用这些默认样式，请将示例中的 `info` 替换为任何这些类型，比如 `[!tip]` 或 `[!warning]`。也可以在实时预览模式下右键点击标注来更改标注类型。
