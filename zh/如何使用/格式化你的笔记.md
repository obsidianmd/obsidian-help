...
在笔记中插入其他文件（更多细节请阅读[[内嵌文件]]）。这里将演示如何插入一篇笔记的某个章节：
...
Markdown 风格的链接既可以用来指向网页这类外部对象，也可以用来指向库中的笔记或者图片。如果url中含有空格的话，空格会被转化为 `%20`，如[Export options](Pasted%20image)。当然，你也可以用`<>`将空格包含在内，如[Slides Demo](<Slides Demo>).
