...
---
Obsidian 的 [安全页面](https://obsidian.md/zh/security) 汇总了 Obsidian 如何保护用户数据的相关信息，同时也是第三方安全审计结果的展示平台。
...
Obsidian 会自动扫描[官方社区市场](https://obsidian.md/plugins)中每个社区插件或主题的每个版本，以检测安全漏洞、代码质量问题和恶意软件。每个项目的市场页面会以安全评分卡的形式展示扫描结果。对于热门、精选和被标记的项目，我们仍会进行人工审核。我们不审核未提交到社区市场的项目。

目前，我们社区市场不提供 [[CSS 代码片段]]。这些文件通常来自 [Obsidian 社区](https://obsidian.md/zh/community) 或公共的 GitHub 仓库。
...
Obsidian 会向 `github.com` 和 `raw.githubusercontent.com` 发起网络请求。
...
目前没有。这可能是我们未来会探索的方向，但目前我们专注于我们的 [安全审计](https://obsidian.md/zh/security)。
