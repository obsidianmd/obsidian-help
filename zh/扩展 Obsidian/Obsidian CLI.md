...
升级到最新的 [[更新 Obsidian|Obsidian 安装程序版本]]（1.12.7+）。
...
- 确保使用的是最新的 [[更新 Obsidian|Obsidian 安装程序版本]]（1.12.7 或更高）。
- 如果你刚从早期版本更新了 Obsidian，请先关闭 CLI 设置再重新开启，然后允许 Obsidian 执行自动 PATH 注册。
- 注册 CLI 后请重启终端，以使 PATH 更改生效。
- Obsidian 必须正在运行。CLI 连接到正在运行的 Obsidian 实例。
...
CLI 注册会在 `/usr/local/bin/obsidian` 创建指向应用内置 CLI 二进制文件的符号链接。此操作需要管理员权限——系统会弹出对话框提示你。
...
> [!note] 如果你之前使用旧版 Obsidian 注册过 CLI，`~/.zprofile` 中可能残留了 PATH 条目。新的注册流程会自动移除它，但如果仍然存在，你可以安全地从 `~/.zprofile` 中删除以 `# Added by Obsidian` 开头的行。

### Linux

CLI 注册会将 CLI 二进制文件复制到 `~/.local/bin/obsidian`。这样做是因为某些 Linux 安装方式从临时目录运行，无法持久创建符号链接。

确保 `~/.local/bin` 在你的 PATH 中。如果不在，请将以下内容添加到 `~/.bashrc` 或 `~/.zshrc`：
...
如果二进制文件缺失，请从 Obsidian 安装目录手动复制：
