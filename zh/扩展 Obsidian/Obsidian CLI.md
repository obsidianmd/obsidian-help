...
用于 [[Introduction to Bases|数据库]] 的命令。
...
移动或重命名文件（默认：活动文件）。如果在[[Settings#始终更新内部链接|仓库设置]]中启用了相关选项，将自动更新[[内部链接]]。
...
重命名文件（默认：活动文件）。如果新名称中省略了文件扩展名，将自动保留原扩展名。使用 [[#`move`|move]] 可同时重命名和移动文件。如果在[[Settings#始终更新内部链接|仓库设置]]中启用了相关选项，将自动更新[[内部链接]]。
