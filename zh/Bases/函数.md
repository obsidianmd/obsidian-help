...
数据库函数遵循 JavaScript 行为。完整参考文档请参阅 [MDN Web Docs](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference)。
...
- 在执行日期运算时不需要显式解析（例如 `now() + '1d'`），但在对时间段执行运算时则需要（例如 `now() + (duration('1d') * 2)`）。
...
- `condition` 是要求值的表达式。
- `trueResult` 是 `condition` 为真时的输出。
- `falseResult` 是 `condition` 为假时的可选输出。如果省略，则默认为 `null`。
...
- 尝试将提供的值作为数字返回。
- 将日期对象作为自 Unix 纪元以来的毫秒数返回。
- 将布尔值作为 `1` 或 `0` 返回。
- 将字符串解析为数字，如果字符串不是有效数字则返回错误。
...
- `replacement` 是用于替换找到的模式的值。当 `pattern` 是正则表达式时，可以在 `replacement` 中使用 `$1`、`$2` 等引用捕获组。
...
- 捕获组示例：`"John Smith".replace(/(\w+) (\w+)/, "$2, $1")` 返回 `"Smith, John"`。
...
- 通过为每个元素运行表达式，将列表归约为单个值。表达式必须返回 `acc` 的下一个值。使用 `value` 表示当前元素，`index` 表示其位置，`acc` 表示到目前为止的累积结果。
...
- 如果 `link` 所代表的文件有指向 `file` 的链接，则返回 `true`。
