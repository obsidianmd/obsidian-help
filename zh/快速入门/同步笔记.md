...
更多替代同步选项可在[社区插件目录](https://community.obsidian.md/search?type=plugin&categories=syncing)中找到。
...
- **启用 iCloud Drive**：
    - 在 macOS 上：前往**系统设置 → Apple ID → iCloud → iCloud Drive**。
...
- **在 iCloud 中创建新仓库**：
    ...
    - 在 iOS 上：
        ...
        3. 开启**存储在 iCloud 中**。
        4. 点击**创建**。
- **保持仓库文件夹已下载**（可选，但强烈建议以保持快速加载）：
    - 对于 **macOS 15（Sequoia）及更高版本**：右键点击 iCloud Drive 中的 **Obsidian** 文件夹，选择**保持下载**。
    - 对于 **macOS 14（Sonoma）及更早版本**：在 iCloud 设置中禁用**优化 Mac 储存空间**。此设置影响设备上的所有 iCloud 存储，而不仅仅是 Obsidian。
- **在另一台 Apple 设备上打开仓库**：
