...
Interpreter 利用语言模型来处理网页上的信息，并使用[[Variables|变量]]返回结果，你可以将这些变量添加到你的 [[Obsidian Web Clipper/模板|Web Clipper 模板]]中。

![[web-clipper-interpreter-demo.mp4#interface]]

## 提示词示例

提示词使用[[Variables|变量]]语法 `{{"your prompt"}}`。你可以将此语法与任何自然语言查询一起使用，例如：
...
提示词的输出可以使用[[Filters|筛选器]]进一步处理。筛选器在从模型接收到提示词响应后进行处理。例如：`{{"a summary of the page"|blockquote}}` 会将响应转换为引用块。
...
3. 配置你的提供商和模型，参见下方的[[Interpret web pages#模型|模型]]部分。
4. 将[[Variables|提示词变量]]添加到你的[[Obsidian Web Clipper/模板|模板]]中。
5. 如果你的模板包含提示词变量，当你[[Clip web pages|剪藏页面]]时，Interpreter 部分将可见。点击 **interpret** 来处理提示词变量。

## 工作原理

当 Interpreter 已启用*且*你的模板包含[[Variables#提示变量|提示词变量]]时，扩展窗口中会在 **Add to Obsidian** 按钮上方显示一个新的 Interpreter 部分。此部分允许你选择模型并为当前页面运行 Interpreter。
...
要定义更有针对性的上下文，可以使用[[Variables#选择器变量|选择器变量]]（或其他变量类型）来解释页面的某个部分。例如，你可以在模板的 Interpreter 上下文中使用以下选择器变量：

```
{{selectorHtml:#main}}
```

这将仅在网页的 `#main` 元素上运行 Interpreter（如果存在的话）。像 `remove_html`、`strip_tags` 和 `strip_attr` 这样的 [[Filters#HTML 处理|HTML 处理筛选器]]可以进一步减少上下文长度并加快处理速度。
...
- 在模板中使用[[#上下文]]字段来提供页面中更有针对性的部分，或使用[[Filters|筛选器]]裁剪上下文，例如 `{{content|slice:0,1000}}`。
