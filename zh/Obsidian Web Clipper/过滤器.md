...
aliases:
  ...
  - Filters
---
筛选器允许你修改 [[Obsidian Web Clipper/模板|Web Clipper 模板]]中的[[variables|变量]]。筛选器通过 `{{variable|filter}}` 语法应用于变量。

- 筛选器适用于任何类型的[[variables|变量]]，包括 `prompt`、`meta`、`selector` 和 `schema` 变量。
...
将字符串、数组或对象转换为 Markdown 链接语法（不要与 [[Filters#`wikilink`|wikilink]] 混淆）。
...
处理 HTML 内容并将 HTML 转换为 Markdown。请注意，输入的[[variables|变量]]必须包含 HTML 内容，例如使用 `{{fullHtml}}`、`{{contentHtml}}` 或 `{{selectorHtml:}}` 变量。
