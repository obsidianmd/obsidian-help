...
[[Obsidian Web Clipper/模板|Web Clipper 模板]]可以使用变量来自动在模板中预填充页面数据。变量可以用在**笔记名称**、**笔记位置**、**属性**和**笔记内容**中。变量还可以通过[[Filters|筛选器]]进行修改。

使用 [[Introduction to Obsidian Web Clipper|Web Clipper]] 扩展中的 `…` 图标可以访问当前页面的变量，以便在模板中使用。你可以使用五种类型的变量：

- [[Variables#预设变量|预设变量]]
- [[Variables#提示变量|提示变量]]
- [[Variables#Meta 变量|Meta 变量]]
- [[Variables#选择器变量|选择器变量]]
- [[Variables#Schema.org 变量|Schema.org 变量]]

## 预设变量

预设变量根据页面内容自动生成。它们通常适用于大多数网站。

主要的内容变量是 `{{content}}`，它包含文章内容、[[Highlight web pages|高亮内容]]，或者页面上选中的文本。请注意，`{{content}}` 会尝试提取页面的主要内容，这可能并不总是你想要的。在这种情况下，你可以使用其他预设变量或选择器变量来提取所需内容。
...
提示变量利用语言模型，通过自然语言提取和修改数据。提示变量需要启用并配置 [[Interpret web pages|解释器]]。

提示变量使用 `{{"页面的摘要"}}` 语法。提示周围的双引号很重要，它将提示与预设变量区分开来。提示响应可以通过[[Filters|筛选器]]进行后处理，例如 `{{"页面的摘要"|blockquote}}`。

### 何时使用提示变量

提示变量的优点是极其灵活且易于编写，但也有一些权衡：它们运行较慢，并且根据你选择的[[Interpret web pages#模型|提供商]]，可能涉及费用和隐私方面的考虑。

与其他变量类型不同，提示变量需要由外部语言模型处理，因此只有在 [[Interpret web pages|解释器]] 运行后才会被替换。
...
提示可以将页面内容转换为 JSON，然后通过[[Filters|筛选器]]进行处理。例如：
...
- `{{selectorHtml:body|markdown}}` 返回 `body` 元素的完整 HTML，通过 `markdown` [[Filters#HTML 处理|筛选器]]转换为 Markdown。
- 如果需要更高的精确度，支持嵌套 CSS 选择器和组合器。
- 如果多个元素匹配选择器，将返回一个数组，你可以使用 `join` 或 `map` 等[[Filters#数组与对象|数组和对象筛选器]]进行处理。
