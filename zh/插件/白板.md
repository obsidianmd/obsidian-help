...
白板是一个用于笔记可视化的工具。将你的笔记和其他资源平铺在一块无限大小的白板上，并用线将他们连接起来。
...
可视化的笔记记录方式，允许你使用不同的显示方式，比如大小或者位，来有效地组织你的笔记。你可以用线将笔记连接起来，并将关联的笔记打组，以此来更好地理清他们之间的关系。
...
- 在左侧的工具栏中，选择 `新建白板`（四个小长方形的图标）来在当前文件夹中新建一个白板。
...
1. 打开 [[命令面板]]。
2. 
3. Select **Canvas: Create new canvas** to create a canvas in the same folder as the active file.
