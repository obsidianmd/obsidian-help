...
Các hàm Cơ sở tuân theo hành vi của JavaScript. Để tham khảo tài liệu đầy đủ, hãy xem [MDN Web Docs](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference).

Ngoài các hàm [[Hàm#Toàn cục|Toàn cục]], hầu hết các hàm phụ thuộc vào kiểu giá trị bạn muốn thao tác:

- [[Hàm#Kiểu Bất kỳ|Bất kỳ]]
- [[Hàm#Kiểu Ngày|Ngày]]
- [[Hàm#Kiểu Chuỗi|Chuỗi]]
- [[Hàm#Kiểu Số|Số]]
- [[Hàm#Kiểu Danh sách|Danh sách]]
- [[Hàm#Kiểu Liên kết|Liên kết]]
- [[Hàm#Kiểu Tệp|Tệp]]
- [[Hàm#Kiểu Đối tượng|Đối tượng]]
- [[Hàm#Kiểu Biểu thức chính quy|Biểu thức chính quy]]
...
- Không cần phân tích rõ ràng khi thực hiện số học ngày (ví dụ, `now() + '1d'`), nhưng cần thiết khi thực hiện phép tính trên khoảng thời gian (ví dụ, `now() + (duration('1d') * 2)`).
...
- `condition` là biểu thức cần đánh giá.
- `trueResult` là kết quả nếu `condition` đúng.
- `falseResult` là kết quả tùy chọn nếu `condition` sai. Nếu bị bỏ qua, giá trị mặc định là `null`.
- Trả về `trueResult` nếu `condition` là đúng hoặc truthy, ngược lại trả về `falseResult`.
...
- Trả về đối tượng hình ảnh hiển thị hình ảnh trong chế độ xem.
...
- Tùy chọn cung cấp tham số `display` để đặt văn bản hiển thị của liên kết.
...
- Sử dụng hàm này khi một thuộc tính chứa hỗn hợp chuỗi hoặc danh sách trong kho.
...
- Trả về đối tượng ngày cho thời điểm hiện tại.
...
- Trả về đối tượng ngày dưới dạng mili giây kể từ Unix epoch.
- Trả về boolean dưới dạng `1` hoặc `0`.
- Phân tích chuỗi thành số, và trả về lỗi nếu chuỗi không phải là số hợp lệ.
...
- Trả về đối tượng ngày cho ngày hiện tại. Phần thời gian được đặt về nửa đêm.
...
- Trả về phần thời gian dưới dạng chuỗi.
...
- `replacement` là giá trị thay thế cho các mẫu tìm thấy. Khi `pattern` là Regexp, bạn có thể tham chiếu các nhóm bắt trong `replacement` bằng `$1`, `$2`, v.v.
...
- Ví dụ với nhóm bắt: `"John Smith".replace(/(\w+) (\w+)/, "$2, $1")` trả về `"Smith, John"`.
...
- Lọc danh sách và chỉ giữ lại các phần tử mà biểu thức trả về true.
...
- Biến đổi mỗi phần tử của danh sách bằng một biểu thức.
- `value` là giá trị của một mục trong danh sách.
- `index` là chỉ số của giá trị hiện tại.
...
- Rút gọn danh sách thành một giá trị duy nhất bằng cách chạy biểu thức cho mỗi phần tử. Biểu thức phải trả về giá trị tiếp theo của `acc`. Sử dụng `value` cho phần tử hiện tại, `index` cho vị trí của nó, và `acc` cho kết quả tích lũy cho đến nay.
...
- Trả về `true` nếu tệp được đại diện bởi `link` có liên kết đến `file`.
...
- `display` là văn bản hiển thị tùy chọn cho liên kết.
...
- Trả về `true` nếu tệp có thuộc tính đã cho.
