...
Bases-funksjoner følger JavaScript-oppførsel. For fullstendig referansedokumentasjon, se [MDN Web Docs](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference).
...
- [[Funksjoner#Datotype|Dato]]
- [[Funksjoner#Strengtype|Streng]]
- [[Funksjoner#Talltype|Tall]]
- [[Funksjoner#Listetype|Liste]]
- [[Funksjoner#Lenketype|Lenke]]
- [[Funksjoner#Filtype|Fil]]
- [[Funksjoner#Objekttype|Objekt]]
- [[Funksjoner#Regulært uttrykk-type|Regulært uttrykk]]
...
- Analyserer en streng som en varighet. Se [[Bases-syntaks#Datoaritmetikk|datoaritmetikk-seksjonen]] for `value`-strengformatet.
- Eksplisitt analysering er ikke nødvendig for datoaritmetikk (for eksempel `now() + '1d'`), men er nødvendig ved aritmetikk på varigheter (for eksempel `now() + (duration('1d') * 2)`).
...
- `condition` er uttrykket som skal evalueres.
- `trueResult` er resultatet hvis `condition` er sann.
- `falseResult` er det valgfrie resultatet hvis `condition` er usann. Hvis utelatt, er standardverdien `null`.
- Returnerer `trueResult` hvis `condition` er sann eller en sann verdi, ellers returneres `falseResult`.
...
- Angi eventuelt `display`-parameteren for å sette visningsteksten for lenken.
...
- Bruk denne funksjonen når en egenskap inneholder en blanding av strenger eller lister på tvers av hvelvet.
...
- Returnerer et datoobjekt for gjeldende tidspunkt.
...
- Returnerer datoobjekter som millisekunder siden Unix-epoken.
- Returnerer boolske verdier som `1` eller `0`.
- Analyserer strenger som tall, og returnerer en feil hvis strengen ikke er et gyldig tall.
...
- Returnerer et datoobjekt for gjeldende dato. Tidsdelen er satt til midnatt.
...
- `replacement` er verdien å erstatte funne mønstre med. Når `pattern` er et regulært uttrykk, kan du referere til fangstgrupper i `replacement` med `$1`, `$2` og så videre.
...
- Eksempel med fangstgrupper: `"John Smith".replace(/(\w+) (\w+)/, "$2, $1")` returnerer `"Smith, John"`.
...
- Valgfritt kan du angi en `digits`-parameter for å runde til det antallet desimalplasser.
...
- Filtrerer listen og beholder bare elementer der uttrykket er sant.
...
- Flater ut en nestet liste til en enkelt liste.
...
- Transformerer hvert element i listen ved hjelp av et uttrykk.
- `value` er verdien av et element i listen.
- `index` er indeksen til gjeldende verdi.
...
- Reduserer listen til en enkelt verdi ved å kjøre et uttrykk for hvert element. Uttrykket må returnere neste verdi av `acc`. Bruk `value` for gjeldende element, `index` for dets posisjon, og `acc` for det akkumulerte resultatet så langt.
...
- Returnerer `true` hvis filen representert av `link` har en lenke til `file`.

## Filtype

Funksjoner du kan bruke med en fil i hvelvet.
...
- `display` er valgfri visningstekst for lenken.
...
- Returnerer `true` hvis filen har den angitte egenskapen.
