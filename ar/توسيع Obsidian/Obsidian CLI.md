...
يقوم تسجيل واجهة سطر الأوامر بنسخ ثنائي واجهة سطر الأوامر إلى `~/.local/bin/obsidian`. يتم ذلك لأن بعض طرق تثبيت Linux تعمل من أدلة مؤقتة لا يمكن إنشاء روابط رمزية دائمة لها.

تأكد من أن `~/.local/bin` موجود في PATH الخاص بك. أضف ما يلي إلى `~/.bashrc` أو `~/.zshrc` إذا لم يكن كذلك:
