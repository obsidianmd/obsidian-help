---
permalink: credits
---
השמות אינם מסודרים לפי כמות התרומה, אלא בסדר אלפביתי (או לפחות אנחנו מנסים!).

## צוות ליבה

צוות הליבה שבונה ומתחזק את Obsidian.

- codedmart (Brandon Martin)
- dharam (Dharam Kapila)
- kepano ([Steph Ango](https://stephanango.com/))
- Licat (Shida Li)
- liam ([Liam Cain](https://liamca.in/)) <span class='flair'>Maker of Calendar</span> <span class='flair'>Plugin API Master</span> <span class='flair'>Volcano Veteran</span>
- Silver ([Erica Xu](https://ericaxu.com))<span class='flair'>Team Little Silvers</span>
- Sigrunixia ([Rebbecca Bishop](https://tenebrousdragon.com/home))<span class='flair'>Sorcerous Support Dragon</span> <span class='flair'>TTRPG Enthusiast</span>
- tgrosinger ([Tony Grosinger](https://grosinger.net))

## תורמים

מפתחים במשרה חלקית שעוזרים לבנות את Obsidian.

- cmptr (Aaron Bell)
- mgmeyers ([Matthew Meyers](https://matthewmeye.rs/))

## סקירת קהילה

תורמים לתהליך סקירת תוספים/ערכות נושא של [הקהילה](https://community.obsidian.md/) ולכלים הקשורים.

- Fevol <span class='flair'>Plugin API Journeyman</span><span class='flair'>Canonically Commentated</span>
- mnaoumov ([Michael Naumov](https://mnaoumov.dev/))
- saberzero1 ([Emile Bangma](https://emilebangma.com)) <span class='flair'>Quartz Quartermaster</span> <span class='flair'>ESLint Legislator</span>
- Zachatoo ([Zach Young](https://zachyoung.dev/)) <span class='flair'>Templater Templar</span><span class='flair'>Board Gamer</span>

## ניהול

מנחים מנהלים את הפורום, Discord ו-Reddit של Obsidian.

- argentum<span class='flair'>Team Little Silvers</span> <span class='flair'>Linux master</span> <span class='flair'>Zotero master</span> ^a4b3a2
- CawlinTeffid ([Colin Tedford](https://colintedford.com/)) <span class='flair'>embedded-search ambassador</span>
- cotemaxime ([Maxime Côté](https://www.maximecote.me/))<span class='flair'>Linux master</span>
- koala<span class='flair'>Extreme Bug Finder</span>
- Leah ([Leah Ferguson](http://leahferguson.com))<span class='flair'>Roll for initiative!</span> <span class='flair'>Non-techy Apple geek</span>
- rigmarole ([Chris Lesage](http://rigmarolestudio.com))<span class='flair'>Forum master</span>
- Roam ([Roman Soriano](https://leftovers.blog/)) <span class='flair'>Not <i>that</i> one</span>
- Trainbuffer <span class='flair'> ferroequinologist</span> <span class='flair'>Travels by rail</span>
- WhiteNoise<span class='flair'>Think Tank</span> <span class='flair'>Zotero master</span> <span class='flair'>Bug report exterminator</span>
- Carl the Turtle<span class='flair'>Online 24/7</span> <span class='flair'>Easily triggered</span> <span class='flair'>Good and pure</span> 

## בוגרים

מנחים וחברי צוות לשעבר שעברו להרפתקאות חדשות!

- Eleanor ([Eleanor Konik](https://eleanorkonik.com))<span class='flair'>Halp Obsidian turned me into a dev</span>
- joethei ([Johannes Theiner](https://joethei.xyz/)) <span class='flair'>Plugin API Master</span>
- mediapathic ([Steen Comer](http://mediapathic.net/))<span class='flair'>Bad Cop</span>
- ryanjamurphy ([Ryan](https://fulcra.design/) [J. A. Murphy](https://axle.design/))<span class='flair'>That funny guy</span> <span class='flair'>Apple Fanboy</span> <span class='flair'>Forum master</span>

## השראה מתוספים

קרדיט למפתחי התוספים הבאים על חלוציות בפונקציונליות שאומצה ב-Obsidian:

- pjeby עבור תוסף Hotkey Helper
- javalent עבור תוסף Admonition
- javalent עבור תוסף Settings Search
- NothingIsLost עבור שיפורי CodeMirror 6
- mgmeyers עבור תוסף Indentation Guides
- blacksmithgu עבור Dataview

## בנייה

- jstone עבור תחזוקת הבניות שלנו ב-Flathub
- adamgibbins ו-ran-dall עבור תחזוקת הבניות שלנו ב-homebrew

## מתרגמים

ראו גם את הרשימה המלאה של [תורמי התרגום](https://github.com/obsidianmd/obsidian-translations/graphs/contributors?from=7%2F31%2F2020).

- Andrea Brandi (איטלקית)
- Anne-Katrin aka Mephi (גרמנית)
- bongudth (וייטנאמית)
- canzi-teacher, 蚕子 (סינית מפושטת)
- cor-bee, Ілля Пишняк (אוקראינית)
- cvyl, Mikka (הולנדית)
- Daniel Mathiot (צרפתית)
- Diego Tres (פורטוגזית ברזילאית)
- Haitham Mohamed (ערבית)
- Henrik Falk (דנית)
- Joffrey Schroeder (צרפתית) 
- JxhnnyUt8h (רוסית)
- k-andzhanovskii, Константин Анджановский (רוסית)
- lisachev, Сергей Лисачев (רוסית)
- mafsi, Patrick Danilevici (רומנית)
- Manuel Arias Almada (ספרדית)
- Moyf, 墨一 (סינית מפושטת)
- Muhammad Mauli Mubassari (אינדונזית)
- Olivier Anthonioz (צרפתית) 
- Ryooo，睿悠ryooo (סינית מפושטת)
- Sakvipubp Suy (חמרית)
- smilorent, Silviu Lorenț (רומנית)
- Solyta Teng (חמרית)
- taksas, 中山 拓海 (יפנית)
- VKondakoff, Валерий Кондаков (רוסית)
- yo-goto, ぱだわん (יפנית)
- Zachary Shirmohammadli (פרסית) 

## ארגונים תומכים

ארגונים התומכים ב-Obsidian עם [[רישיון פרסומי|רישיון מסחרי]] מוצגים בדף ה-[Enterprise](https://obsidian.md/enterprise/) שלנו. 

## תיעוד

הגרסה הראשונה של כספת העזרה נכתבה על ידי Silver. חלק מאוחר יותר נכתב על ידי [@mediapathic](http://mediapathic.net), בעזרה גדולה מ-Reggie. קיבלנו גם תיקונים מעיניים רבות בקהילת Discord והפורום.

התיעוד תוחזק על ידי [Marcus Olsson](https://marcus.se.net/) למשך תקופה. כיום הוא מתוחזק על ידי צוות Obsidian.

אנו מקבלים כעת בקשות משיכה (pull requests) לכספת העזרה ב-[מאגר התיעוד שלנו](https://github.com/obsidianmd/obsidian-docs/). דיווחי בעיות מתקבלים בברכה.

### הוסף את שמך

אם אתה מתרגם, אנא שלח בקשת משיכה כדי להוסיף "השם שלך (שפת התרומה)" לקטע למעלה. אנא שמור על הרשימה בסדר אלפביתי, תודה!

אם שמך אינו באנגלית ולא ניתן לסדרו אלפביתית, השתמש בפורמט הזה: `שם-משתמש-github-שלך, [השם שלך בשפתך] (שם השפה)`.

## הכרת תודה לצדדים שלישיים

Obsidian משתמש בספריות קוד פתוח הבאות (בסדר אלפביתי):

### Capacitor

גרסה `5.x`
MIT License
Copyright (c) 2017-present Drifty Co.

### CodeMirror

Vim גרסה `6.0.0`
MIT License
Copyright (C) 2018 by Marijn Haverbeke <marijnh@gmail.com>, Adrian Heine <mail@adrianheine.de>, and others

### DOMPurify

https://github.com/cure53/DOMPurify
Licensed under the [Mozilla Public License version 2.0](http://mozilla.org/MPL/2.0/)

### Electron

גרסה `43.1.1`
MIT License
Copyright (c) Electron contributors
Copyright (c) 2013-2020 GitHub Inc.

### i18next

MIT License
Copyright (c) 2022 i18next

### Lezer

MIT License
Copyright (C) 2018-2021 by Marijn Haverbeke <marijnh@gmail.com> and others

### Lucide

גרסה `0.446.0`
ISC License
Copyright (c) 2020, Lucide Contributors ^lucide

### MathJax

Apache License 2.0

### Mermaid

גרסה `11.13.0`
MIT License
Copyright (c) 2014 - 2022 Knut Sveidqvist

### Moment.js

גרסה `2.30.1`
MIT License
Copyright (c) JS Foundation and other contributors

### pdf.js

Apache License 2.0

### PixiJS

MIT License
Copyright (c) 2013-2017 Mathew Groves, Chad Engler

### Prism

גרסה `1.29.0`
MIT License
Copyright (c) 2012 Lea Verou

### rbush

MIT License
Copyright (c) 2016 Vladimir Agafonkin

### remark

MIT License
Copyright (c) 2014-2020 Titus Wormer <tituswormer@gmail.com>
Copyright (c) 2011-2014, Christopher Jeffrey (https://github.com/chjj/)

### reveal.js

גרסה `4.3.1`
The MIT License
Copyright (C) 2011-2022 Hakim El Hattab, http://hakim.se, and reveal.js contributors

### scrypt

Apache License 2.0

### Turndown

MIT License
Copyright (c) 2017 Dom Christie

### Webpack

MIT License
Copyright JS Foundation and other contributors

### YAML

גרסה `2.7.0`
ISC License
Copyright Eemeli Aro <eemeli@gmail.com>

## סמל קודם

סמל Obsidian שהיה בשימוש לפני גרסה v1.3.5 הותאם מ:

[Crystal shard icon](https://icon-icons.com/icon/Crystal-Shard/88819)
על ידי [Chanut is Industries](https://icon-icons.com/users/W52nHhY3W1VlvwyJTwS4d/icon-sets/ "Designer")
רישיון: Attribution 3.0 Unported (CC BY 3.0)
