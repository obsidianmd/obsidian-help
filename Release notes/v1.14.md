...
- The entire workspace, including the sidebars, now flips when Obsidian is set to an RTL language. Commands stay the same, so "Toggle left sidebar" now refers to the visual left sidebar, even when the interface is flipped.
- Closing popout windows (such as Settings) is now much faster.
- Editor: Pasting a task onto a line that already contains an empty task marker (`- [ ] `) no longer duplicates the marker.
