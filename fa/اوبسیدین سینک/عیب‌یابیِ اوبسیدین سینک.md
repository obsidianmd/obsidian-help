...
If a note was created locally on a device less than a couple of minutes before Sync downloads a remote version of that note, then Sync keeps the remote version without attempting to merge the two. You can still recover the local version using [[بازیابیِ پرونده‌ها]].

## What does the `vault limit exceeded` error mean?

Your vault exceeds the [[محدودیت‌ها#How large can each remote vault be|maximum size of 10 GB]].
