...
- Open the [[فرمان‌دان]] and then select **Add internal link**.

While you can link to any of the [[قالب‌های پذیرفتنی برای پرونده‌ها]], links to file formats other than Markdown needs to include a file extension, such as `[[Figure 1.png]]`.

## پیوند به سرفصلی درونِ یادداشت
...
For example, `[[Internal links|custom display text]]` appears as [[پیوندهای درونی|custom display text]].

**Markdown format:**

Enter the display text between the square brackets (`[]`).

For example, `[custom display text](Internal%20links.md)` appears as [custom display text](پیوندهای%20درونی.md).

## Preview a linked file

> [!note]
> To preview linked files, you first need to enable [[پیش‌نمایشِ صفحه]].
