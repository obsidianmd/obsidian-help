...
## افزودنِ دگرنام به یک یادداشت

To add an alias for a note, add an `alias`, or `aliases`, property in the note [[فراداده|front matter]]:
...
## پیوند زدن به یک یادداشت به‌یاریِ دگرنام

To link to a note using an alias:

1. Start typing the alias in an [[پیوندهای درونی|internal link]]. Any alias shows up in the list of suggestions, with a curved arrow icon next to it.
...
By using [[پشت‌وندها]], you can find unlinked mentions of aliases.

For example, after setting "AI" as an alias for "Artificial intelligence", you can see mentions of "AI" in other notes.

If you link an unlinked mention to an alias, Obsidian turns the mention into an [[پیوندهای درونی|internal link]] with the alias as its display text.
