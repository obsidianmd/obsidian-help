---
permalink: credits
---
Os nomes não são listados por quantidade de contribuição, mas alfabeticamente (ou pelo menos tentamos!).

## Equipe principal

A equipe principal que constrói e mantém o Obsidian.

- codedmart (Brandon Martin)
- dharam (Dharam Kapila)
- kepano ([Steph Ango](https://stephanango.com/))
- Licat (Shida Li)
- liam ([Liam Cain](https://liamca.in/)) <span class='flair'>Maker of Calendar</span> <span class='flair'>Plugin API Master</span> <span class='flair'>Volcano Veteran</span>
- Silver ([Erica Xu](https://ericaxu.com))<span class='flair'>Team Little Silvers</span>
- Sigrunixia ([Rebbecca Bishop](https://tenebrousdragon.com/home))<span class='flair'>Sorcerous Support Dragon</span> <span class='flair'>TTRPG Enthusiast</span>
- tgrosinger ([Tony Grosinger](https://grosinger.net))

## Contribuidores

Desenvolvedores de meio período que ajudam a construir o Obsidian.

- cmptr (Aaron Bell)
- mgmeyers ([Matthew Meyers](https://matthewmeye.rs/))

## Revisão da comunidade

Contribuidores para o processo e ferramentas de revisão de plugins/temas da [Comunidade](https://community.obsidian.md/).

- Fevol
- mnaoumov ([Michael Naumov](https://mnaoumov.dev/))
- Saberzero1 (Emile Bangma)
- Zachatoo ([Zach Young](https://zachyoung.dev/)) <span class='flair'>Templater Templar</span><span class='flair'>Board Gamer</span>

## Moderação

Os moderadores gerenciam o Fórum, Discord e Reddit do Obsidian.

- argentum<span class='flair'>Team Little Silvers</span> <span class='flair'>Linux master</span> <span class='flair'>Zotero master</span> ^a4b3a2
- CawlinTeffid ([Colin Tedford](https://colintedford.com/)) <span class='flair'>embedded-search ambassador</span>
- cotemaxime ([Maxime Côté](https://www.maximecote.me/))<span class='flair'>Linux master</span>
- koala<span class='flair'>Extreme Bug Finder</span>
- Leah ([Leah Ferguson](http://leahferguson.com))<span class='flair'>Roll for initiative!</span> <span class='flair'>Non-techy Apple geek</span>
- rigmarole ([Chris Lesage](http://rigmarolestudio.com))<span class='flair'>Forum master</span>
- Roam ([Roman Soriano](https://leftovers.blog/)) <span class='flair'>Not <i>that</i> one</span>
- Trainbuffer <span class='flair'> ferroequinologist</span> <span class='flair'>Travels by rail</span>
- WhiteNoise<span class='flair'>Think Tank</span> <span class='flair'>Zotero master</span> <span class='flair'>Bug report exterminator</span>
- Carl the Turtle<span class='flair'>Online 24/7</span> <span class='flair'>Easily triggered</span> <span class='flair'>Good and pure</span> 

## Alumni

Moderadores e membros da equipe anteriores que seguiram para novas aventuras!

- Eleanor ([Eleanor Konik](https://eleanorkonik.com))<span class='flair'>Halp Obsidian turned me into a dev</span>
- joethei ([Johannes Theiner](https://joethei.xyz/)) <span class='flair'>Plugin API Master</span>
- mediapathic ([Steen Comer](http://mediapathic.net/))<span class='flair'>Bad Cop</span>
- ryanjamurphy ([Ryan](https://fulcra.design/) [J. A. Murphy](https://axle.design/))<span class='flair'>That funny guy</span> <span class='flair'>Apple Fanboy</span> <span class='flair'>Forum master</span>

## Inspirações de plugins

Créditos a estes desenvolvedores de plugins por terem sido pioneiros em funcionalidades que foram adaptadas para o Obsidian:

- pjeby pelo plugin Hotkey Helper
- javalent pelo plugin Admonition
- javalent pelo plugin Settings Search
- NothingIsLost pelas melhorias no CodeMirror 6
- mgmeyers pelo plugin Indentation Guides
- blacksmithgu pelo Dataview

## Compilações

- jstone por manter nossas compilações no Flathub
- adamgibbins e ran-dall por manterem nossas compilações no homebrew

## Tradutores

Veja também a lista completa de [contribuidores de tradução](https://github.com/obsidianmd/obsidian-translations/graphs/contributors?from=7%2F31%2F2020).

- Andrea Brandi (Italiano)
- Anne-Katrin aka Mephi (Alemão)
- bongudth (Vietnamita)
- canzi-teacher, 蚕子 (Chinês Simplificado)
- cor-bee, Ілля Пишняк (Ucraniano)
- cvyl, Mikka (Holandês)
- Daniel Mathiot (Francês)
- Diego Tres (Português Brasileiro)
- Haitham Mohamed (Árabe)
- Henrik Falk (Dinamarquês)
- Joffrey Schroeder (Francês) 
- JxhnnyUt8h (Russo)
- k-andzhanovskii, Константин Анджановский (Russo)
- lisachev, Сергей Лисачев (Russo)
- mafsi, Patrick Danilevici (Romeno)
- Manuel Arias Almada (Espanhol)
- Moyf, 墨一 (Chinês Simplificado)
- Muhammad Mauli Mubassari (Indonésio)
- Olivier Anthonioz (Francês) 
- Ryooo，睿悠ryooo (Chinês Simplificado)
- Sakvipubp Suy (Khmer)
- smilorent, Silviu Lorenț (Romeno)
- Solyta Teng (Khmer)
- taksas, 中山 拓海 (Japonês)
- VKondakoff, Валерий Кондаков (Russo)
- yo-goto, ぱだわん (Japonês)
- Zachary Shirmohammadli (Persa) 

## Organizações apoiadoras

Organizações que apoiam o Obsidian com uma [[Licença comercial]] são apresentadas em nossa página [Enterprise](https://obsidian.md/enterprise/). 

## Documentação

A primeira versão do cofre de ajuda foi escrita por Silver. Parte posterior foi escrita por [@mediapathic](http://mediapathic.net), com grande ajuda de Reggie. Também recebemos correções dos muitos olhos atentos na comunidade do Discord e do fórum.

A documentação foi mantida por [Marcus Olsson](https://marcus.se.net/) por um tempo. Atualmente é mantida pela equipe do Obsidian.

Agora aceitamos pull requests para o cofre de ajuda em [nosso repositório de documentação](https://github.com/obsidianmd/obsidian-docs/). Issues são bem-vindos.

### Adicione seu nome

Se você é um tradutor, por favor faça um pull request para adicionar "Seu Nome (idioma contribuído)" à seção acima. Por favor mantenha a lista de nomes em ordem alfabética, obrigado!

Se seu nome não está em inglês e não pode ser listado alfabeticamente, use este formato: `seu-nome-de-usuario-github, [seu nome no seu idioma] (Nome do idioma)`.

## Agradecimentos a terceiros

O Obsidian utiliza as bibliotecas de código aberto abaixo (em ordem alfabética):

### Capacitor

Versão `5.x`
Licença MIT
Copyright (c) 2017-present Drifty Co.

### CodeMirror

Vim Versão `6.0.0`
Licença MIT
Copyright (C) 2018 by Marijn Haverbeke <marijnh@gmail.com>, Adrian Heine <mail@adrianheine.de>, and others

### DOMPurify

https://github.com/cure53/DOMPurify
Licenciado sob a [Mozilla Public License versão 2.0](http://mozilla.org/MPL/2.0/)

### Electron

Versão `37.3.0`
Licença MIT
Copyright (c) Electron contributors
Copyright (c) 2013-2020 GitHub Inc.

### i18next

Licença MIT
Copyright (c) 2022 i18next

### Lezer

Licença MIT
Copyright (C) 2018-2021 by Marijn Haverbeke <marijnh@gmail.com> and others

### Lucide

Versão `0.446.0`
Licença ISC
Copyright (c) 2020, Lucide Contributors ^lucide

### MathJax

Apache License 2.0

### Mermaid

Versão `11.4.1`
Licença MIT
Copyright (c) 2014 - 2022 Knut Sveidqvist

### Moment.js

Versão `2.29.4`
Licença MIT
Copyright (c) JS Foundation and other contributors

### pdf.js

Apache License 2.0

### PixiJS

Licença MIT
Copyright (c) 2013-2017 Mathew Groves, Chad Engler

### Prism

Versão `1.29.0`
Licença MIT
Copyright (c) 2012 Lea Verou

### rbush

Licença MIT
Copyright (c) 2016 Vladimir Agafonkin

### remark

Licença MIT
Copyright (c) 2014-2020 Titus Wormer <tituswormer@gmail.com>
Copyright (c) 2011-2014, Christopher Jeffrey (https://github.com/chjj/)

### reveal.js

Versão `4.3.1`
Licença MIT
Copyright (C) 2011-2022 Hakim El Hattab, http://hakim.se, and reveal.js contributors

### scrypt

Apache License 2.0

### Turndown

Licença MIT
Copyright (c) 2017 Dom Christie

### Webpack

Licença MIT
Copyright JS Foundation and other contributors

### YAML

Versão `2.7.0`
Licença ISC
Copyright Eemeli Aro <eemeli@gmail.com>

## Logo anterior

O logo do Obsidian usado antes da v1.3.5 foi adaptado de:

[Crystal shard icon](https://icon-icons.com/icon/Crystal-Shard/88819)
Por [Chanut is Industries](https://icon-icons.com/users/W52nHhY3W1VlvwyJTwS4d/icon-sets/ "Designer")
Licença: Attribution 3.0 Unported (CC BY 3.0)
