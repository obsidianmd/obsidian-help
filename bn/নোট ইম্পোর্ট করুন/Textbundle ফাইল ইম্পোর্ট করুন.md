---
permalink: import/textbundle
aliases:
  - Import Textbundle files
cssclasses:
  - soft-embed
---
Obsidian আপনাকে অফিসিয়াল [[ইম্পোর্টার|Importer প্লাগইন]] ব্যবহার করে সহজেই [Textbundle](https://textbundle.org/) ফাইল ইম্পোর্ট করতে দেয়।

আপনি যদি কোনো নির্দিষ্ট অ্যাপ থেকে নোট ইম্পোর্ট করছেন, তাহলে প্রথমে [[ইম্পোর্টার|Importer প্লাগইন]] দ্বারা সমর্থিত অ্যাপের তালিকা পর্যালোচনা করে দেখে নিতে পারেন যে কোনো নির্দিষ্ট কনভার্টার আছে কিনা যা আপনার মাইগ্রেট করা ডেটা আরও ভালোভাবে সংরক্ষণ করবে।

Textbundle ফরম্যাট Markdown টেক্সট এবং সমস্ত রেফারেন্স করা ছবি একটি একক ফাইলে বান্ডেল করে, যা একটি স্যান্ডবক্স করা অ্যাপ্লিকেশন থেকে বের হয়ে আসার আরও সহজ উপায় প্রদান করে। Textbundle এক্সপোর্ট [অনেক অ্যাপ](https://textbundle.org/) দ্বারা সমর্থিত, যার মধ্যে রয়েছে Agenda, Craft, Taio, Ulysses, Zettlr এবং আরও অনেক কিছু।

## Obsidian-এ Textbundle ফাইল ইম্পোর্ট করুন

আপনার অফিসিয়াল Obsidian [[ইম্পোর্টার]] প্লাগইন প্রয়োজন হবে, যা আপনি [এখানে ইনস্টল করতে পারেন](obsidian://show-plugin?id=obsidian-importer)।

1. **[[সেটিংস]]** খুলুন।
2. **Community Plugins**-এ যান এবং [Importer ইনস্টল করুন](obsidian://show-plugin?id=obsidian-importer)।
3. Importer প্লাগইন চালু করুন।
4. কমান্ড প্যালেট বা রিবন আইকন ব্যবহার করে **Importer** প্লাগইন খুলুন।
5. **File format**-এর অধীনে **Textbundle (.textbundle, .textpack)** নির্বাচন করুন।
6. আপনার Textbundle বা Textpack ফাইলের অবস্থান নির্বাচন করুন। জিপ ফাইলও সমর্থিত।
7. তৈরি করা টেমপ্লেট পর্যালোচনা করতে এবং আপনার নোটগুলো থেকে প্রিভিউ উদাহরণ দেখতে **Import** নির্বাচন করুন।
8. আবার **Import** নির্বাচন করুন এবং ইম্পোর্ট সম্পূর্ণ হওয়া পর্যন্ত অপেক্ষা করুন।

## ইম্পোর্ট করা নোট কাস্টমাইজ করুন

ইম্পোর্ট শুরু হওয়ার আগে, Importer নির্বাচিত Textbundle ফাইলগুলো থেকে তৈরি একটি প্রিভিউ দেখায়। আপনি তৈরি করা টেমপ্লেটটি সম্পাদনা করতে পারেন অথবা আপনার ভল্ট থেকে একটি Markdown টেমপ্লেট নির্বাচন করতে পারেন। দেখুন [[ইম্পোর্টার টেমপ্লেট]]।

## সমস্যা সমাধান

### ইম্পোর্টের জন্য কোনো ফাইল নির্বাচিত হয়নি

Textbundle ইম্পোর্ট একবারে শুধুমাত্র একটি Textbundle ফাইল ইম্পোর্ট করতে পারে, এবং শুধুমাত্র Mac ডিভাইসে। এর পরিবর্তে, আপনি ইম্পোর্ট করতে চান এমন সমস্ত Textbundle ফাইল একটি একক .zip ফাইলে সংকুচিত করে যেকোনো ডিভাইস থেকে এটি ইম্পোর্ট করতে পারেন।

## টেমপ্লেট

আপনার ডেটা কীভাবে ইম্পোর্ট করা হবে তা সম্পূর্ণরূপে কনফিগার করতে [[ইম্পোর্টার টেমপ্লেট|Importer templates]] ব্যবহার করুন।

![[ইম্পোর্টার টেমপ্লেট#Variables]]
