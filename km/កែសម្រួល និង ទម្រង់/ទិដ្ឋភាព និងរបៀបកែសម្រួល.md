...
> [!note] Note
> By default, Obsidian opens new tabs in editing mode. You can change this under **[[ការកំណត់]] → Editor → Default view for new tabs**.
...
> [!note] Note
> Enable **[[ការកំណត់]] → Appearance → Show tab title bar** to see the view switcher icon.  
> 
> If that is disabled, turn on **[[ការកំណត់]] → Editor → Show editing mode in status bar** to toggle views from the status bar.
...
- Or press `Ctrl+E` (`Cmd+E` on macOS).
- Or use the [[ក្ដារលាយពាក្យបញ្ជា|command]] **Toggle Reading view**.

> [!note] Note
> By default, editing view is set to *Live Preview*. You can change this under **[[ការកំណត់]] → Editor → Default editing mode**.
...
> [!note] Note
> By default, editing view is set to *Live Preview*. Change this to *Source mode* under **[[ការកំណត់]] → Editor → Default editing mode**.
> 
...
> To toggle between *Live Preview* and *Source mode* quickly, you can set a [[ផ្លូវកាត់ក្តារចុច|Hotkey]] for the [[ក្ដារលាយពាក្យបញ្ជា|command]] **Toggle Live Preview/Source mode**.
