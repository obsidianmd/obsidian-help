...
---
Note composer is a [[កម្មវិធីជំនួយចម្បង|core plugin]] that lets you merge two notes or extract part of a note into a new note.
...
> [!tip] Tip
> By default, Note composer asks you to confirm when merging notes. If you disable the confirmation, and you merge a note by mistake, you can still recover it with the [[ការស្តារឯកសារឡើងវិញ]] plugin.
...
1. While in the **Editing view**, select the text you want to extract.
2. Open the [[ក្ដារលាយពាក្យបញ្ជា]].
3. Select **Note composer: Extract current selection...**.
4. Select the note you want to extract into.

> [!tip] Tip
> By default, Note composer replaces the extracted text with a link to the destination note. Under settings, you can also change to instead [[បង្កប់ឯកសារ|embed]] the destination note, or to leave nothing behind.
