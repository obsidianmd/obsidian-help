...
Search is a [[កម្មវិធីជំនួយចម្បង|core plugin]] that helps you find data in your Obsidian vault by using search terms and operators to narrow down results.
...
> [!info] Excluded files
> Files matching your [[ការកំណត់#Excluded files|Excluded files]] patterns will not appear in Search results.
...
| `task:`         | Find matches in a [[វាក្យសម្ពន្ធទម្រង់មូលដ្ឋាន#Task lists\|task]] on a block-by-block basis.<p/>Example: `task:call`.                                                                                                                                                                                                 |
| `task-todo:`    | Find matches in an *uncompleted* [[វាក្យសម្ពន្ធទម្រង់មូលដ្ឋាន#Task lists\|task]] on a block-by-block basis.<p/>Example: `task-todo:call`.                                                                                                                                                                             |
| `task-done:`    | Find matches in a *completed* [[វាក្យសម្ពន្ធទម្រង់មូលដ្ឋាន#Task lists\|task]] on a block-by-block basis.<p/>Example: `task-done:call`.                                                                                                                                                                                |

## Search properties

You can use data stored in [[លក្ខណៈសម្បត្តិ]] in your search terms.
...
[[ការណែនាំអំពី Obsidian Publish|Obsidian Publish]] doesn't support embedded [[ដែនកំណត់របស់ Publish#ការស្វែងរក|search results]]. To see a live rendered example, use the code block above within your vault.
