...
**[[ការកំណត់]]** → **[[ការកំណត់#Files and links|Files and links]]** → **[[ការកំណត់#Automatically update internal links|Automatically update internal links]]**.
...
While you can link to any of the [[ទម្រង់ឯកសារដែលទទួលយក]], links to file formats other than Markdown needs to include a file extension, such as `[[Figure 1.png]]`.

> [!tip] Prefixing an internal link with an exclamation mark (!) allows you to embed the linked content. For more details, see [[បង្កប់ឯកសារ]].

> [!info] Excluded files
> Files matching your [[ការកំណត់#Excluded files|Excluded files]] patterns are deprioritized in link suggestions when creating internal links.
...
For example, `[[អំពី Obsidian#តំណភ្ជាប់ជាពលរដ្ឋថ្នាក់ទីមួយ]]` will create a link to [[អំពី Obsidian#តំណភ្ជាប់ជាពលរដ្ឋថ្នាក់ទីមួយ]].

**Linking to subheadings**

You can add multiple hash symbols for each subheading.

For example, `[[ជំនួយ និងការគាំទ្រ#Questions and advice#Report bugs and request features]]` will create a link to [[ជំនួយ និងការគាំទ្រ#Questions and advice#Report bugs and request features]].
...
This method is helpful for one-off situations where you want to change how a link looks in a specific context. If you want to set up an alternate link name that you can reuse throughout your vault, consider using an [[ឈ្មោះក្លែងក្លាយ|alias]] instead. 
...
> 
> Use [[ឈ្មោះក្លែងក្លាយ|aliases]] when you want to refer to the same note using *different names* throughout your vault.
^callout-internal-links-link-text

## Preview a linked file

> [!note] Note
> To preview linked files, you first need to enable [[ការមើលទំព័រជាមុន]].
