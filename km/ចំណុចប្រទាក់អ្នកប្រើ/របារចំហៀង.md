...
Obsidian has two sidebars: a left sidebar and a right sidebar. Sidebars hold tabs created by plugins, such as [[តំណភ្ជាប់ខាងក្រោយ]], [[តំណភ្ជាប់ចេញ]], and the [[កម្មវិធីរុករកឯកសារ]]. On desktop, you can also drag notes into the sidebar to keep them visible alongside your main editor. 

On desktop and larger tablets, the left sidebar includes the [[រីបិន]]. On mobile and smaller tablets, the sidebars are [[#Open hidden sidebars|collapsed by default]]. 
...
- Use the *Toggle left* or *Toggle right* [[ក្ដារលាយពាក្យបញ្ជា]] options. 
...
- **Run a command**: Use the [[ក្ដារលាយពាក្យបញ្ជា]] to search for commands that open specific tabs. For example, search for "Backlinks" or "Outgoing links" to open those tabs.
- **Drag a note** (desktop only): On desktop, you can drag a note from the [[កម្មវិធីរុករកឯកសារ]] or from an editor tab into the sidebar to open it there.
...
- **Panes** (like [[តំណភ្ជាប់ខាងក្រោយ]] or [[តំណភ្ជាប់ចេញ]]): A pinned pane stays focused on the last selected note. Selecting a different note does not update the pinned pane.

For information about pinning tabs in the main editor, see [[ផ្ទាំង#Pin a tab|Pin a tab]].
