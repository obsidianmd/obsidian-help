...
In the [[របារចំហៀង#បើករបារចំហៀងដែលបានលាក់|left sidebar]], select **[[ការកំណត់]]** ![[lucide-cog.svg#icon]]. You can also open Settings with the [[ក្ដារលាយពាក្យបញ្ជា]].

### Mobile

In the left sidebar, select **[[ការកំណត់]]** ![[lucide-cog.svg#icon]].
...
- **[[កម្មវិធីជំនួយចម្បង]]** — Built-in plugins that come with Obsidian.
- **[[កម្មវិធីជំនួយសហគមន៍]]** — Third-party plugins installed from the community.
...
Auto-update to the latest [[កំណែចូលប្រើមុន]]. These versions include new features but may be less stable.

> [!info]+ Catalyst license required
> This option is only available for users with a [[អាជ្ញាប័ណ្ណ catalyst]].

### Language

Select your preferred language for the Obsidian interface. Select **[[ការបកប្រែ|Learn how to add a new language to Obsidian]]** to contribute a translation.

### Help

Select **[[ទំព័រដើម|Open]]** to access help resources, including documentation, community forums, and troubleshooting guides.
...
View your Catalyst license status. If you have an active [[អាជ្ញាប័ណ្ណ catalyst]], your license tier appears here.

**Commercial license**

If you support [Obsidian for work](<https://obsidian.md/blog/free-for-work/>), select **Activate** to enter a commercial license key, or select **[Purchase](https://obsidian.md/account/commercial)** to buy a license. Select **[[អាជ្ញាប័ណ្ណពាណិជ្ជកម្ម|Learn more]]** for information about Commercial licenses.

### Advanced

#### Notify if startup takes longer than expected

When enabled, Obsidian displays a notification if startup is unusually slow. The notification includes diagnostic information about what's causing the delay. Select the timer icon ![[lucide-timer.svg#icon]] next to this setting to [[ជំនួយ និងការគាំទ្រ#ពិនិត្យមើលពេលវេលាចាប់ផ្តើម|check your startup time]].
...
The default view that a new Markdown file tab gets opened in. Choose between [[ទិដ្ឋភាព និងរបៀបកែសម្រួល#ទិដ្ឋភាពអាន|Reading view]] or [[ទិដ្ឋភាព និងរបៀបកែសម្រួល#ទិដ្ឋភាពកែសម្រួល|Editing view]].

### Default editing mode

The default editing mode a new tab will start with. Choose between [[ទិដ្ឋភាព និងរបៀបកែសម្រួល#ការមើលផ្ទាល់|Live preview]] or [[ទិដ្ឋភាព និងរបៀបកែសម្រួល#របៀបប្រភព|Source mode]].

### Show editing mode in status bar

Show the editing mode toggle in the [[របារស្ថានភាព|status bar]].
...
Markdown specs ignore single [[វាក្យសម្ពន្ធទម្រង់មូលដ្ឋាន#ការបំបែកបន្ទាត់|Line breaks]] in reading view. Turn this off to make single line breaks visible.

#### Properties in document

Choose how [[លក្ខណៈសម្បត្តិ|properties]] are displayed at the top of notes. Select **source** to show properties as raw YAML.

#### Fold heading

[[បត់/បង្រួម|Fold]] all content under a heading by selecting the collapse icon next to it.

### Fold indent

[[បត់/បង្រួម|Fold]] part of an indentation, such as lists, by selecting the collapse icon.
...
Where newly added [[ឯកសារភ្ជាប់|attachments]] are placed. Options include:
...
What links to insert when auto-generating [[តំណភ្ជាប់ផ្ទៃក្នុង|internal links]]. Options include:
...
Excluded files will be hidden in [[ស្វែងរក]], [[ទិដ្ឋភាពក្រាហ្វ]], and Unlinked Mentions ([[តំណភ្ជាប់ខាងក្រោយ]] and [[តំណភ្ជាប់ចេញ]]), and less noticeable in [[ឧបករណ៍ផ្លាស់ប្តូររហ័ស]] and link suggestions. Select **Manage** to configure excluded files.

#### Override config folder

Use a different [[ថតកំណត់រចនាសម្ព័ន្ធ|config folder]] than the default one. Must start with a dot.
...
Rebuilds the vault's [[របៀបដែល Obsidian រក្សាទុកទិន្នន័យ#ឃ្លាំងផ្ទុកមេតាដាតា|metadata cache]]. Rebuilding the cache could take a few seconds to a few minutes depending on the size of your vault. Select **Rebuild** to start the process.
...
Manage installed [[រូបរាង|themes]] and browse community themes. Select **Manage** to view and install themes. The dropdown shows your currently active theme. The folder icon ![[lucide-folder-open.svg#icon]] opens the themes folder in your [[ថតកំណត់រចនាសម្ព័ន្ធ|configuration folder]].
...
Display vertical toolbar on the side of the window. Learn more about the [[រីបិន|ribbon]].
...
> On mobile, ribbon customization works differently. See [[រីបិន#ទូរសព្ទ|Ribbon customization on mobile]] for details.
...
Set a custom icon for the app. Select **Choose** to pick an icon file (`.icns`, `.ico`, `.png`, or `.svg`). Learn more about [[រូបរាង#រូបសញ្ញាកម្មវិធីផ្ទាល់ខ្លួន|custom app icons]].
...
> [!info]+ macOS only
> This feature is primarily designed for macOS. Learn more about [[រូបរាង#ភាពថ្លា|translucency]].
...
Manage and view [[ស្នីប៉ែត CSS|CSS snippets]] stored in your vault. CSS Snippets are stored in `/vault/.obsidian/snippets/`. Select the refresh icon to reload snippets, or the folder icon ![[lucide-folder-open.svg#icon]] to open the snippets folder.

## Hotkeys

The [[ផ្លូវកាត់ក្តារចុច]] page allows you to view, set, and customize [[ផ្លូវកាត់កែសម្រួល|keyboard shortcuts]] for commands in Obsidian.
...
[[កម្មវិធីជំនួយចម្បង]] are built-in plugins that come with Obsidian. Each plugin adds specific functionality to the app.

To enable or disable a core plugin, select the toggle next to the plugin name. Select the plus ![[lucide-plus-circle.svg#icon]] icon to view the [[ផ្លូវកាត់ក្តារចុច]] for the plugin. Select the settings ![[lucide-settings.svg#icon]] icon to open the plugin's settings page.

Use the search bar to filter plugins by name.

## Community plugins

[[កម្មវិធីជំនួយសហគមន៍]] are third-party plugins created by the Obsidian community. Browse and install plugins by selecting **Browse**.

### Restricted mode

[[សុវត្ថិភាពកម្មវិធីជំនួយ#របៀបដាក់កម្រិត|Restricted mode]] disables all community plugins for security. Select **Turn on and reload** to enable restricted mode.
