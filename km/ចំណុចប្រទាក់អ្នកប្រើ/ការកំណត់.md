...
In the [[របារចំហៀង#Open hidden sidebars|left sidebar]], select **[[ការកំណត់]]** ![[lucide-cog.svg#icon]]. You can also open Settings with the [[ក្ដារលាយពាក្យបញ្ជា]].
...
When enabled, Obsidian displays a notification if startup is unusually slow. The notification includes diagnostic information about what's causing the delay. Select the timer icon ![[lucide-timer.svg#icon]] next to this setting to [[ជំនួយ និងការគាំទ្រ#Check startup time|check your startup time]].
...
The default view that a new Markdown file tab gets opened in. Choose between [[ទិដ្ឋភាព និងរបៀបកែសម្រួល#Reading view|Reading view]] or [[ទិដ្ឋភាព និងរបៀបកែសម្រួល#Editing view|Editing view]].

### Default editing mode

The default editing mode a new tab will start with. Choose between [[ទិដ្ឋភាព និងរបៀបកែសម្រួល#Live Preview|Live preview]] or [[ទិដ្ឋភាព និងរបៀបកែសម្រួល#Source mode|Source mode]].
...
Markdown specs ignore single [[វាក្យសម្ពន្ធទម្រង់មូលដ្ឋាន#Line breaks|Line breaks]] in reading view. Turn this off to make single line breaks visible.
...
Rebuilds the vault's [[របៀបដែល Obsidian រក្សាទុកទិន្នន័យ#Metadata cache|metadata cache]]. Rebuilding the cache could take a few seconds to a few minutes depending on the size of your vault. Select **Rebuild** to start the process.
...
> On mobile, ribbon customization works differently. See [[រីបិន#Mobile|Ribbon customization on mobile]] for details.
...
Set a custom icon for the app. Select **Choose** to pick an icon file (`.icns`, `.ico`, `.png`, or `.svg`). Learn more about [[រូបរាង#Custom app icon|custom app icons]].
...
> [!info]+ macOS only
> This feature is primarily designed for macOS. Learn more about [[រូបរាង#Translucency|translucency]].
...
[[សុវត្ថិភាពកម្មវិធីជំនួយ#Restricted mode|Restricted mode]] disables all community plugins for security. Select **Turn on and reload** to enable restricted mode.
