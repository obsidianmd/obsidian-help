...
---
Filters allow you to modify [[អថេរ]] in [[Obsidian Web Clipper/ពុម្ព|Web Clipper templates]]. Filters are applied to variables using the syntax `{{variable|filter}}`.

- Filters work for any kind of [[អថេរ|variable]] including `prompt`, `meta`, `selector`, and `schema` variables.
...
Apply [[វាក្យសម្ពន្ធទម្រង់មូលដ្ឋាន]] and [[វាក្យសម្ពន្ធទម្រង់កម្រិតខ្ពស់]] to text.
...
Creates a [[ប្រអប់រំលេច|callout]] with optional parameters: `{{variable|callout:("type", "title", foldState)}}`
...
Converts strings, arrays, or objects into Markdown link syntax (not to be confused with [[តម្រង#`wikilink`|wikilink]]).
...
Converts an array or array of objects into a [[វាក្យសម្ពន្ធទម្រង់កម្រិតខ្ពស់#Tables|Markdown table]]:
...
Converts strings, arrays, or objects into Obsidian [[តំណភ្ជាប់កំណត់ត្រា|wikilink]] syntax.
...
Process HTML content and convert HTML to Markdown. Note that your input [[អថេរ|variable]] must contain HTML content, e.g. using `{{fullHtml}}`, `{{contentHtml}}` or a `{{selectorHtml:}}` variable.
