...
---
A **vault** is a folder on your file system which contains your notes, [[ឯកសារភ្ជាប់]], and the [[ថតកំណត់រចនាសម្ព័ន្ធ]] with Obsidian-specific settings. For more information on vaults, refer to [[របៀបដែល Obsidian រក្សាទុកទិន្នន័យ]].

You can manage your vaults using the **Vault switcher**. The *Vault profile* opens the first time you open Obsidian.

To open the vault switcher from an existing vault, select **Vault profile** ![[lucide-chevrons-up-down.svg#icon]] at the bottom of the [[របារចំហៀង|left sidebar]]. Or, select **Open another vault** from the [[ក្ដារលាយពាក្យបញ្ជា]].
...
> If you want to open a remote vault using Obsidian Sync, refer to [[ដំឡើង Obsidian Sync]].
...
8. Click **Open**.
9. Verify that the vault contents are unchanged. If necessary, re-enable community plugins by navigating to **[[ការកំណត់]] → Community Plugins → Turn restricted mode off**.
...
> By default, most operating systems hide folders starting with a period (`.`). For more information about the `.obsidian` folder and how to access it, refer to [[របៀបដែល Obsidian រក្សាទុកទិន្នន័យ#Vault settings|vault settings]] and [[ថតកំណត់រចនាសម្ព័ន្ធ|config folders]].
