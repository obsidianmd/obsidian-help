...
When referring to multiple UI interactions in a sequence, use the → (U+2192) symbol. For example, "**[[ការកំណត់]] → Community plugins**".
...
- Select **[[ការកំណត់]]** in the bottom-left corner.
- Select **[[ការកំណត់]]** in the bottom left.
...
- [[បង្កើត base]] — Learn how to create and embed a base.
...
First mention: "Learn about [[ការណែនាំអំពី Obsidian Sync|Obsidian Sync]] to keep your vault updated across devices."
 ...
 **Example**: Once enabled, the [[រាប់​ពាក្យ]] plugin will create a new entry on your bottom statusbar.
 
![[Style-guide-zoomed-example.png#interface|300]]

- Images should be in either `.png` or `.svg` format.
- If an image looks too big in the note, make it smaller outside of Obsidian, or adjust its dimensions as explained in [[បង្កប់ឯកសារ#បង្កប់រូបភាពក្នុងកំណត់ត្រា|embedding an image in a note]].
- In rare cases, you may want to place especially large or complex images in a [[ប្រអប់រំលេច#ប្រអប់រំលេចដែលអាចបត់បាន|folded callout]]. 
...
Images slow the loading time of the page, and take valuable [[ការណែនាំអំពី Obsidian Publish|Publish]] storage space. Optimizing images allows a reduction in file size, but maintains the visual integrity of the image. 
...
You can check for broken links using [[កម្មវិធីជំនួយសហគមន៍]] or tools available in your IDE.

### Descriptions

This documentation is edited on GitHub and hosted online via [[ការណែនាំអំពី Obsidian Publish|Obsidian Publish]], which includes [[ការមើលជាមុនតំណភ្ជាប់បណ្ដាញសង្គម#ការពិពណ៌នា|descriptions]] for social cards and other [[SEO]] elements.

If the page you are working on does not have a `description` [[លក្ខណៈសម្បត្តិ|property]], please add one. The description should be 150 characters or fewer and provide an objective summary of the page's content.
