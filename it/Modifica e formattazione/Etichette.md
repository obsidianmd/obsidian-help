...
Per trovare note utilizzando il plugin [[Cerca|Cerca]], usa l'[[Cerca#Operatori di ricerca|operatore di ricerca]] `tag` nel termine di ricerca, ad esempio `tag:#meeting`.

Puoi anche cercare le etichette facendo clic su di esse nelle tue note.

Per trovare note utilizzando il plugin [[Pannello delle etichette|Pannello delle etichette]], seleziona **Tags: Mostra pannello etichette** nella [[Riquadro comandi|tavolozza dei comandi]], quindi seleziona l'etichetta che desideri cercare.
...
- In [[Cerca|Cerca]], `tag:inbox` corrisponderà a `#inbox` così come a tutte le etichette nidificate come `#inbox/to-read`.
- Nel [[Pannello delle etichette|Pannello delle etichette]], le etichette nidificate vengono mostrate come appartenenti alla loro etichetta genitore.
- In [[Introduzione a Base|Base]], le etichette nidificate sono riconosciute dalla funzione [[Funzioni#hasTag|`hasTag`]], quindi `file.hasTag("a")` corrisponderà sia a `#a` che a `#a/b`.
...
> [!note] 
> Le etichette verranno visualizzate con le maiuscole/minuscole con cui sono state create per la prima volta nel [[Pannello delle etichette|Pannello delle etichette]]. 
