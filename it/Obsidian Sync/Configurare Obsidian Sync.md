...
In questa sezione, creerai un nuovo [[Vault locali e remoti|caveau remoto]] e lo collegherai a una cassaforte locale esistente. Non è necessario creare una nuova cassaforte locale vuota per utilizzare Obsidian Sync a questo scopo.

> [!info] La tua cassaforte attuale si trova in una cartella iCloud, OneDrive, Dropbox o di un altro servizio di sincronizzazione? Se **sì**, o **non sei sicuro**, leggi [[Domande frequenti#Posso usare un sync di terze parti con Obsidian Sync?|questo]] e [[Passare a Obsidian Sync]] prima di procedere.

**Prerequisiti**

- Un account Obsidian. Se non ne hai uno, [iscriviti ora](https://obsidian.md/it/auth?returnto=%2Faccount%2Fsync#signup).
- Un [[Piani e limiti di archiviazione|abbonamento]] attivo a Obsidian Sync. Se non ne hai uno, abbonati dalla [dashboard del tuo account](https://obsidian.md/it/account/sync).
- **Consigliato**: un [[Backup dei file di Obsidian|sistema di backup]] attivo per i tuoi file Obsidian. Un servizio di sincronizzazione non è un backup.
...
7. In **Password di crittografia**, scegli una password per il tuo vault. Questo crea un vault con crittografia end-to-end. La password del vault è separata dal tuo account Obsidian e può essere diversa per ciascuno dei tuoi vault. Per ulteriori informazioni, consulta [[Sicurezza e privacy]].
8. Seleziona **Crea**.
...
1. Se necessario, vai su **[[Impostazioni|Impostazioni]]** → **Sync**.
...
5. Una volta riavviato Obsidian, torna su **[[Impostazioni|Impostazioni]]** → **Sync**.
...
>
> Per maggiori dettagli sugli stati di sincronizzazione, consulta [[Icona di stato e messaggi]].
^obsidian-sync-status

Per collegare altri dispositivi al tuo caveau remoto appena creato e sincronizzato, passa a [[Configurare Obsidian Sync#Sincronizzare un caveau remoto su un altro dispositivo|Sincronizzare un caveau remoto su un altro dispositivo]].

Per saperne di più su impostazioni e file, passa a [[Impostazioni sincronizzazione e sincronizzazione selettiva]].
...
- Sync abilitato nelle impostazioni dei [[Plugin principali|Plugin principali]].
- Un caveau remoto attivo. Se non ne hai ancora creato uno, crea prima un [[Configurare Obsidian Sync#Creare un nuovo caveau remoto|caveau remoto]].
- **Consigliato**: un [[Backup dei file di Obsidian|sistema di backup]] attivo per i tuoi file Obsidian sul dispositivo più utilizzato. Un servizio di sincronizzazione non è un backup.

### Sincronizzare la cassaforte dal selettore dei vault

Se hai appena installato Obsidian, quando apri il programma ti verrà presentato il [[Gestisci casseforti|Selettore dei vault]]. Per creare una nuova cassaforte locale dal contenuto di un caveau remoto, dovrai eseguire i seguenti passaggi.
...
3. Accedi con il tuo account utente Obsidian
	1. Se l'[[Autenticazione a 2 fattori|autenticazione a due fattori]] è configurata, inserisci il codice 2FA.
...
![[Configurare Obsidian Sync#Accedere con il tuo account Obsidian]]

![[Configurare Obsidian Sync#Abilitare Obsidian Sync]]
...
> [!warning] Se la cassaforte sul tuo dispositivo contiene già alcune note (non consigliato), verrai avvisato che quelle note verranno unite prima di procedere. I conflitti verranno risolti tramite la [[Risoluzione problemi di Obsidian Sync#Risoluzione dei conflitti|risoluzione dei conflitti di Sync]].

![[Configurare Obsidian Sync#Regolare le impostazioni di Obsidian Sync]]

![[Configurare Obsidian Sync#Iniziare la sincronizzazione con Obsidian Sync]]
...
1. Apri le **[[Impostazioni|Impostazioni]]** di Obsidian.
...
Dopo aver selezionato una posizione, il tuo data center **non può** essere spostato su un server diverso senza ricaricare i tuoi dati. Per cambiare regione, segui la [[Regioni di Sync|guida alle regioni Sync dei vault]].
...
- Scopri di più sulla [[Impostazioni sincronizzazione e sincronizzazione selettiva|selezione dei file e delle impostazioni da sincronizzare]].
- Scopri cosa succede se il tuo caveau remoto si [[Cronologia versioni|riempie]].
- [[Collaborare su un vault condiviso|Collabora su una cassaforte condivisa]] con un altro utente di Obsidian Sync.
- Consulta le [[Domande frequenti|FAQ di Sync]] per alcune risposte alle domande comuni.
