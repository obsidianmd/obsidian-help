...
在[社群外掛目錄](https://community.obsidian.md/search?type=plugin&categories=syncing)中可以找到數十種其他同步選項。
...
- **啟用 iCloud Drive**：
    - macOS：前往**系統設定 → Apple ID → iCloud → iCloud Drive**。
...
- **在 iCloud 中建立新保管庫**：
    ...
    - iOS：
        ...
        3. 開啟**儲存在 iCloud**。
        4. 點按**建立**。
- **保持保管庫資料夾已下載**（選用，但強烈建議以維持快速載入時間）：
    - macOS 15 (Sequoia) 及更新版本：在 iCloud Drive 中的 **Obsidian** 資料夾上按右鍵，選取**保持下載**。
    - macOS 14 (Sonoma) 及更早版本：在 iCloud 設定中停用**最佳化 Mac 儲存空間**。此設定影響裝置上所有的 iCloud 儲存空間，不僅限於 Obsidian。
- **在另一台 Apple 裝置上開啟保管庫**：
